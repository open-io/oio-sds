...
class TestDirectoryFunctional(BaseTestCase):
    ...
    def _reload(self):
        url = self.conf['proxyd_uri'] + '/v2.0/cache/flush/local'
        resp = self.session.post(url, '')
        self.assertEqual(resp.status_code / 100, 2)
        for srvtype in ('meta1', 'meta2'):
            for t in self.conf[srvtype]:
                url = self.conf['proxyd_uri'] + '/v2.0/forward/' + str(t)
    ...
    def url_ref(self, ref):
        u = self.conf['proxyd_uri']
        return '/'.join((u, 'v2.0/dir', self.ns, self.account, ref))

    def url_srvtype(self, t):
        return '/'.join((self.conf['proxyd_uri'], 'v2.0/cs', self.ns, t))
    ...
    def setUp(self):
        ...
        self.service1 = {'type': 'echo', 'ns': self.ns, 'score': 100,
                         'addr': self.addr1, 'tags': {'tag.vol': 'test',
                                                      'tag.up': True}}
        self.service2 = {'type': 'echo', 'ns': self.ns, 'score': 100,
                         'addr': self.addr2, 'tags': {'tag.vol': 'test',
                                                      'tag.up': True}}
        ...
        url = self.url_srvtype("echo") + "/action"
        resp = self.session.post(url, json.dumps(args))
        ...
        action = {'action': 'SetProperties', 'args': {'prop1': self.property3,
                                                      'prop2': self.property4}}
        resp = self.session.post(self.url_ref(self.ref1) + '/action',
                                 json.dumps(action))
        self.assertEqual(resp.status_code / 100, 2)
        ...
        resp = self.session.post(self.url_ref(self.ref1) + '/echo/action',
                                 json.dumps(action))
    ...
    def test_references_actions_GetProperties_Invalid(self):
        ...
        resp = self.session.post(self.url_srvtype("echo") +
                                 "/action/no/such/resource",
                                 json.dumps(action))
        self.assertEqual(resp.status_code, 404)

        action = {'action': 'GetProperties', 'args': None}
        resp = self.session.post(self.url_srvtype("unknown"),
                                 json.dumps(action))
        self.assertEqual(resp.status_code, 404)

    def test_references_actions_setProperties(self):
        action = {'action': 'SetProperties', 'args': {'prop1': self.property1,
    ...
    def test_references_actions_setProperties_invalid(self):
        action = {'action': 'SetProperties', 'args': {'prop1': self.property1,
                                                      'prop2': self.property2}}
        resp = self.session.post(self.url_ref("error") + '/action',
                                 json.dumps(action))
        self.assertEqual(resp.status_code, 404)
        resp = self.session.post(self.url_ref(self.ref1) +
                                 '/action/no/such/resource',
                                 json.dumps(action))
        self.assertEqual(resp.status_code, 404)
    ...
    def test_services_actions_link(self):
        action = {"action": "Link", "args": None}
        resp = self.session.post(self.url_ref(self.ref2) + '/echo/action',
                                 json.dumps(action))
        self.assertEqual(resp.status_code, 200)
    ...
    def test_service_actions_link_again(self):
        ...
        resp = self.session.post(self.url_srvtype("echo") + "/action",
                                 json.dumps(action))
        self.assertEqual(resp.status_code, 200)
    ...
    def test_service_actions_force_replace_with_header(self):
        ...
        self._reload()
        action = {"action": "Force", "args": {"seq": 1, "type": "echo",
        ...
        action = {"action": "Force", "args": {"seq": 1, "type": "echo",
                                              "host": self.addr2, "args": ""}}
        h = {'x-oio-action-mode': 'replace'}
        resp = self.session.post(self.url_ref(self.ref2) + '/echo/action',
                                 json.dumps(action), headers=h)
        self.assertEqual(resp.status_code, 204)
