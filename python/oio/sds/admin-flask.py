...
def create_container(ns, ref):
    try:
        ...
        # Links a meta2
        r = session.post(
            ENDPOINT + '/v1.0/dir/{0}/{1}/meta2/action'.format(ns, ref),
...
def move_container(ns, ref):
    try:
        ...
        # Freeze the source
        r = session.post(
            ENDPOINT + '/v1.0/sqlx/{0}/{1}/meta2/1/action'.format(ns, ref),
        ...
        if r.status_code / 100 == 2:
            ...
            if r.status_code / 100 == 2:
                ...
                r = session.post(
                    ENDPOINT +
                    '/v1.0/sqlx/{0}/{1}/meta2/1/action'.format(ns, ref),
...
@app.route('/v1.0/admin/election/<ns>/<ref>/<srvtype>',
           methods=['GET', 'DELETE'])
