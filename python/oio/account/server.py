...
with DB() as db:
    db.execute(
        "CREATE TABLE IF NOT EXISTS items(key BLOB PRIMARY KEY, value BLOB NOT NULL)")
...
account_fields = ['name', 'ctime',
                  'user-count', 'container-count', 'content-count',
                  'bytes-count',
...
def list_prefixed_items(cursor, prefix):
    cursor.execute(
        "SELECT key, value FROM items WHERE key >= ? ORDER BY key ASC",
        (prefix,))
...
def account_list_users(ns, account):
    with DB() as db:
        ...
        return flask.Response('[' + ",".join(result) + ']',
                              mimetype='text/json')
...
def user_list_containers(ns, account, user):
    with DB() as db:
        ...
        result = list()
        for key, _ in list_prefixed_items(cursor,
                                          key_user(account, user) + '|'):
            tokens = key.split('|')
            if len(tokens) != 3:
                continue
            result.append(tokens[2])
...
@app.route('/v1.0/account/<ns>/<account>/<user>/<container>',
           methods=['GET', 'HEAD'])
