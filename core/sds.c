 ...
 */
static void oio_sds_ec_cache_destroy_handle(struct oio_sds_ec_s *handle)
{
	if  (handle->ec_handle) {
		int res = liberasurecode_instance_destroy(handle->ec_handle);
		if (res) {
			// TODO(jfs): A smarter error message is necessary
			GRID_WARN("[%s] Unable to destroy EC handle (%s)",
					__func__, "liberasurecode_instance_destroy");
		}
	}
	handle->ec_handle = 0;
	g_free(handle);
