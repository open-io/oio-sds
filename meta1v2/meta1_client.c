...
{
	gchar **result = NULL;
	GError *err = meta1v2_remote_get_prefixes(addr, &result);
	...
	if (err != NULL) {
		...
		if ( result == NULL || g_strv_length(result) == 0) {
			GRID_WARN("NO prefix managed by this meta1 %s.", addr);
			return;
		}
		guint i=0, done=0, len=g_strv_length(result);
		...
		g_print("\n");
		GRID_INFO("This meta1 %s managed %d prefixes", addr, len);
	}
}
...
static void meta1_specific_fini(void) { }
...
meta1_configure(int argc, char **argv)
{
	...
	if (!g_ascii_strcasecmp(argv[1], "list")) {
		GRID_WARN("Invalid command, see usage.");
		return FALSE;
	}

	g_strlcpy(addr, argv[0], sizeof(addr));
	return TRUE;
}

static void meta1_specific_stop(void) { }
