...
{
	char str_score[16], str_addr[STRLEN_ADDRINFO];
	...
	gboolean show_internals)
{
	...
	for (l = services; l; l = l->next) {
		...
		if(show_internals || !service_info_is_internal(si)) {
			grid_addrinfo_to_string(&(si->addr), str_addr, sizeof(str_addr));
			g_snprintf(str_score, sizeof(str_score), "%d", si->score.value);
			g_print("%s,%s,%s,%d", ns ?: si->ns_name,
					type ?: si->type, str_addr, si->score.value);
			if (si->tags) {
				int i, max;
...
set_service_score(const char *service_desc, int score)
{
	...
	GError *err = conscience_register_service (si);
	if (err)
...
main(int argc, char **argv)
{
	...
	while ((c = getopt_long(argc, argv, "TALsvearBC:S:h", long_options, &option_index)) > -1) {
	...
	if (has_lbconfig) {
		...
		error = conscience_clear_services (namespace, service_desc, NULL);
		if (error) {
	...
	else if (has_srvtypes) {
		GSList *types = NULL;
		error = conscience_list_service_types (namespace, &types);
		if (error) {
			g_printerr("Failed to list the service tyes for [%s]\n", namespace);
			goto exit_label;
		}
		...
		error = conscience_list_service_types (namespace, &services_types);
		...
		if (!services_types) {
			...
			for (GSList *st = services_types; st; st = st->next) {
				...
				error = conscience_list_services (namespace, str_type,
						!has_flag_full || !has_raw, FALSE, &list_services);
