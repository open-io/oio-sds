...
func fillBuffer(src io.Reader, buf []byte, reqid string) (written int, err error) {
	for len(buf)-written >= defs.UploadBatchSize {
		LogInfo("Reading source (reqid=%s len_buf=%d written=%d UploadBatchSize=%d)", reqid, len(buf), written, defs.UploadBatchSize)
		nr, er := src.Read(buf[written:])
		LogInfo("Source read (reqid=%s nr=%d er=%v)", reqid, nr, er)
		if nr > 0 {
			written += nr
		}
		if er != nil {
