...
> Delay between each poll when sending a sync event (in ms). Total duration is this value multiplied by oio_events_kafka_sync_max_polls.
...
### events.kafka.timeout.shutdown

> Set the Kafka client flush timeout on events queue when shutting down

 * default: **10 * G_TIME_SPAN_SECOND**
 * type: gint64
 * cmake directive: *OIO_EVENTS_KAFKA_TIMEOUT_SHUTDOWN*
 * range: 0 -> 1 * G_TIME_SPAN_DAY
