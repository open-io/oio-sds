...
void rainx_init_content_hash()
{
	MD5_Init(&content_md5_ctx);
	bzero(&content_hash, sizeof(content_hash));
}

content_hash_t* rainx_finalize_content_hash()
{
...
static GError *
_extract_size_and_md5(gchar *header, chunk_size_t *new_size, chunk_hash_t *new_md5)
{
	GError *err = NULL;
...
_find_header(chunk_id_t chunkid, gchar **split_header)
{
	...
	for (cursor = split_header; NULL != *cursor; cursor++) {
		...
		hex2bin(header_chunkid_str, header_chunk_id, sizeof(header_chunk_id), NULL);
		// cleanup
		g_strfreev(header_tokens);
		...
		GError **err)
{
	...
	void _update_chunk_info(gpointer _chunk_info, gpointer _unused)
	{
		...
		updated_list = g_slist_append(updated_list, g_memdup(chunk_info, sizeof(chunk_info_t)));
	}
...
gs_status_t rainx_upload (GSList *chunk_list, gs_content_t *hollow_content,
		...
		const gchar *storage_policy, guint32 current_metachunk_pos, guint32 current_metachunksize, GError **err)
{
	ne_session *session = NULL;
	ne_request *request = NULL;
	gchar *str_req_id = NULL;
	...
	if (NULL == (str_req_id = create_rainx_request_from_chunk_list(&request, session, RAINX_UPLOAD,
			chunk_list, hollow_content, system_metadata, storage_policy, current_metachunk_pos, current_metachunksize, err))) {
		goto error_label;
	}

	/*tell we'll read data from the provided callback*/
	ne_set_request_body_provider (request, size_to_be_read, limited_input, user_data);
	...
	switch (ne_request_dispatch (request))
	{
		case NE_OK:
			if (ne_get_status(request)->klass != 2) {
				GSETERROR(err, "cannot upload (%s) (ReqId:%s)", ne_get_error(session), str_req_id);
				goto error_label;
			}
		...
		case NE_TIMEOUT:
		case NE_ERROR:
			GSETERROR(err, "error from the RAINX WebDAV server (%s) (ReqId:%s)", ne_get_error(session), str_req_id);
			goto error_label;
		default:
			GSETERROR(err, "unexpected error from the RAINX WebDAV server (%s) (ReqId:%s)", ne_get_error(session), str_req_id);
			goto error_label;
	}

	/* The response header "chunklist" contains size and md5sum. */
	if (NULL != (header_chunklist = ne_get_response_header(request, "chunklist"))) {
		GRID_DEBUG("header_chunklist=%s", header_chunklist);
		updated_chunk_list = _update_chunk_list(chunk_list, header_chunklist, err);
		if (err && *err) {
			goto error_label;
		}
		*p_returned_chunk_list = g_slist_concat(*p_returned_chunk_list, updated_chunk_list);
	} else {
		GSETERROR(err, "Could not find chunklist header in response (ReqId:%s)", str_req_id);
		goto error_label;
	}
...
error_label:
	...
	if (session)
		ne_session_destroy (session);
	TRACE("could not upload %s (ReqId:%s)", hollow_content->info.path, str_req_id);
	g_free(str_req_id);
	return FALSE;
}
