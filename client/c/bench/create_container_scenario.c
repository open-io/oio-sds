...
#ifndef G_LOG_DOMAIN
#define G_LOG_DOMAIN "grid.client.bench.create_container_scenario"
#endif
...
gboolean create_container_scenario(struct scenario_data * sdata, gchar * result_str, gint result_str_len)
{
	...
	g_snprintf(result_str, result_str_len, "CREATE %s", container_name);
	container = gs_get_storage_container(sdata->gs, container_name, NULL, 1, &error);
	if (container != NULL) {
