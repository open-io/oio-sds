...
#ifndef G_LOG_DOMAIN
#define G_LOG_DOMAIN "grid.client.bench.get_service_scenario"
#endif
...
gboolean get_service_scenario(struct scenario_data * sdata, gchar * result_str, gint result_str_len)
{
	...
	g_snprintf(result_str, result_str_len, "LIST SERVICES [%s] from container %s", SERVICE_NAME, container_name);
	...
	services = gs_container_service_get_all(container, SERVICE_NAME, &error);
	if (services == NULL && error) {
		GRID_ERROR("Get services [%s] for container %s failed with error : %s", SERVICE_NAME, container_name, gs_error_get_message(error));
		gs_error_free(error);
