...
struct arg_{{f.name}}_s {
	...
	{{END-}}
	{{IF f.return.serializer}}{{f.return.type}} result;{{END-}}
};
...
msg_manager_{{f.name}}(GError ** err, gpointer udata, gint code, MESSAGE rep)
{
	...
	if (code==206 && 0>message_has_BODY(rep,NULL)) {
		GSETERROR(err,"No body found in the reply");
		goto error_label;
	} else { /*a body has been found in the message*/
...
{{f.return.type}}
{{f.prefix}}_remote_{{f.name}}(struct metacnx_ctx_s *ctx, {{FOREACH arg IN f.args}}{{arg.type}} {{IF arg.is_out}}*{{END}}{{arg.local_name}}, {{END}}GError **err)
{
	MESSAGE request;
	...
	{{FOREACH arg IN f.out_args}}args.{{arg.local_name}} = NULL;
	{{END-}}
	{{IF f.return.serializer}}args.result = NULL;{{END-}}
	
	if (!ctx{{FOREACH arg IN f.in_args}} || !{{arg.local_name}}{{END}}) {
		GSETERROR(err,"Invalid parameter ({{FOREACH arg IN f.in_args}} {{arg.local_name}}=%p{{END}})"
			{{FOREACH arg IN f.in_args}}, (void*){{arg.local_name}}{{END}});
		goto error_check;
	}
	...
	{{FOREACH arg IN f.in_args}}{{IF arg.serializer}}
	do {
		int rc;
		GByteArray *gba;
		
		{{SWITCH arg.serializer}}
		{{CASE constants.SERIALIZE_STRUCT}}gba = g_byte_array_append(g_byte_array_new(), (guint8*)&{{arg.local_name}}, sizeof({{arg.type}}));
		{{CASE constants.SERIALIZE_ARRAY}}gba = g_byte_array_append(g_byte_array_new(), (guint8*){{arg.local_name}}, sizeof({{arg.type}}));
		{{CASE constants.SERIALIZE_POINTER}}gba = g_byte_array_append(g_byte_array_new(), (guint8*){{arg.local_name}}, sizeof(*{{arg.local_name}}));
		{{CASE constants.SERIALIZE_STRING}}gba = g_byte_array_append(g_byte_array_new(), (guint8*){{arg.local_name}}, strlen({{arg.local_name}}));
		{{CASE}}gba = {{arg.marshaller}}({{arg.local_name}},err);
		{{END}}
		if (!gba) {
		...
		{{SWITCH arg.message_location}}
		{{CASE 'body'}}rc = message_set_BODY(request, gba->data, gba->len, err);
		g_byte_array_free(gba, TRUE);
		{{CASE 'header'}}rc = message_add_field(request, "{{arg.message_name}}", sizeof("{{arg.message_name}}")-1, gba->data, gba->len, err);
		g_byte_array_free(gba, TRUE);
		{{END}}
		if (!rc) {
...
error_check:
	...
	{{IF f.return.serializer-}}
	{{IF f.return.singleton}}if (!args.result)
		return 0;
	else {
		{{f.return.type}} result_element;
		result_element = args.result->data;
		g_slist_free(args.result);
		return result_element;
	}
	{{ELSE}}return args.result;{{END}}
	{{ELSE}}return status;
	{{END}}
}
