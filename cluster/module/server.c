		...
		service_callback_f *callback, gpointer u)
{
	...
	while (g_hash_table_iter_next(&iter, &key, &value)) {
		...
		if (!p_srv->put_locked && p_srv->get_locked && p_srv->put_score.timestamp < oldest) {
			if (p_srv->put_score.value > 0 || p_srv->get_score.value > 0) {
		...
		struct service_info_dated_s *sid)
{
	...
	if (sid->lock_mtime > p_srv->lock_mtime) {
		...
		/* Set a tag to reflect the locked/unlocked state of the service.
		 * Modifying service_info_s would cause upgrade issues. */
		tag_lock = service_info_ensure_tag(p_srv->tags, NAME_TAGNAME_PUT_LOCK);
		service_tag_set_value_boolean(tag_lock, p_srv->put_locked);
	}
