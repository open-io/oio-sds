...
m2db_replace_shard_ranges(struct sqlx_sqlite3_s *sq3, GSList *new_shard_ranges)
{
	...
	for (GSList *l = new_shard_ranges; !err && l; l = l->next) {
		err = _db_save_bean(sq3, l->data);
	}
	if (!err) {
...
m2db_clean_shard(struct sqlx_sqlite3_s *sq3)
{
	...
	err = _db_delete(&descr_struct_PROPERTIES, sq3, "alias IN ("
			"SELECT properties.alias FROM properties "
			"LEFT JOIN aliases ON (properties.alias = aliases.alias "
			"AND properties.version = aliases.version)"
			"WHERE aliases.alias IS NULL)", NULL);
	if (err) {
		goto end;
	}
	// Remove orphan contents
	err = _db_delete(&descr_struct_CONTENTS_HEADERS, sq3, "id IN ("
			"SELECT contents.id FROM contents "
			"LEFT JOIN aliases ON contents.id = aliases.content "
			"WHERE aliases.content IS NULL)", NULL);
	if (err) {
		goto end;
	}
	// Remove orphan chunks
	err = _db_delete(&descr_struct_CHUNKS, sq3, "content IN ("
			"SELECT chunks.content FROM chunks "
			"LEFT JOIN contents ON chunks.content = contents.id "
			"WHERE contents.id IS NULL)", NULL);
	if (err) {
		goto end;
	}
