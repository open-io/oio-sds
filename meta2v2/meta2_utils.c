...
m2db_transition_policy(struct sqlx_sqlite3_s *sq3, struct oio_url_s *url,
		struct namespace_info_s* nsinfo UNUSED, GSList** transitioned, gboolean* updated,
		const gchar *new_policy)
{
	...
	GError *err = NULL;
	gchar* current_policy = NULL;
	gchar* target_policy = NULL;
	struct storage_policy_s* pol = NULL;
	gboolean emit_transition_event = TRUE;
	struct bean_ALIASES_s *current_alias = NULL;
	struct bean_CONTENTS_HEADERS_s *current_header = NULL;
	...
	*updated = FALSE;
	...
	m2v2_policy_decode(
		CONTENTS_HEADERS_get_policy(current_header), &current_policy, &target_policy);

	// Ensure new policy is not already applied
	if (g_strcmp0(new_policy, target_policy) == 0) {
		goto cleanup;
	}
	if (g_strcmp0(new_policy, current_policy) == 0) {
	...
	gchar* old_policy = current_policy;
	if (target_policy != NULL) {
		old_policy = target_policy;
	}
	// Update bean
	gint64 size = CONTENTS_HEADERS_get_size(current_header);
	const gchar* new_policy_str = m2v2_policy_encode(old_policy, new_policy);
	CONTENTS_HEADERS_set2_policy(current_header, new_policy_str);
	err = _db_save_bean(sq3, current_header);
	// Decrement old policy
	m2db_update_size(sq3, -size, old_policy);
	m2db_update_obj_count(sq3, -1, old_policy);
	...
	*updated = TRUE;
	if (emit_transition_event) {
		*transitioned = g_slist_prepend(*transitioned, current_header);
		current_header = NULL;
	}
