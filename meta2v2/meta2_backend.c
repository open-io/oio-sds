...
static void
m2b_close(struct sqlx_sqlite3_s *sq3, struct oio_url_s *url)
{
	if (!sq3)
		return;
		...
		struct beanstalkd_s **pbeanstalkd)
{
	...
	err = beanstalkd_factory(beanstalkd_endpoint, tube, &beanstalkd);
	g_free(tube);
	if (err)
		return err;
		...
		m2_onbean_cb cb, gpointer u0, gchar ***out_properties)
{
	...
	if (100 > total || total > 100) {
		err = BADREQ("Invalid partition: total must be exactly 100");
		goto end;
	}
	if (jthreshold) {
