		...
		struct oio_url_s *url, json_object *jparams, guint32 *incr_offset)
{
	...
	gchar *base_query = "SELECT al.alias, al.version, al.content, al.deleted, al.mtime, m_ct.policy FROM aliases AS al "
				"INNER JOIN contents m_ct on al.content = m_ct.id";
	gchar *base_query_versioned = "SELECT al.alias, al.version, al.content, al.deleted, al.mtime, "
				"m_ct.policy, COUNT(*) AS nb_versions, MAX(al.version) FROM versioned_view AS al "
				"INNER JOIN contents m_ct on al.content = m_ct.id";

	gchar *base_query_marker = "SELECT al.alias, al.version, al.content, al.deleted, al.mtime, "
