...
class Generator(object):
    ...
    def dump_c_header(self, out):
        ...
        for t in self.allbeans.values():
            out.write(
                "extern const struct bean_descriptor_s descr_struct_"
                + t.name.upper()
                + ";\n"
            )
        ...
        for t in self.allbeans.values():
            ...
            out.write(
                "\nGError* "
                + u
                + "_load(struct sqlx_sqlite3_s *sq3, const gchar *clause,\n"
            )
            ...
            for f in t.fields:
                ...
                if isinstance(f, Blob):
                    out.write(
                        ...
                        + "(struct bean_"
                        + u
                        + "_s *bean, const guint8 *v, gsize vlen);\n"
                    )
                if not f.mandatory:
                    out.write(
                        "void "
    ...
    def dump_c_codec(self, out):
        ...
        for t in self.reverse_dependencies():
            ...
            for fk in t.fk_outgoing:
                ...
                tmp.append(
                    ...
                    + fk.target.sql_name
                    + "("
                    + ",".join([n for i, n in fk.target_fields])
                    + ")"
                )
        ...
        for fk in self.allfk:
            out.write(
                "static struct fk_field_s descr_fk_fields_in_"
                + fk.name.upper()
                + "[] =\n{\n\t"
            )
            out.write(
                ", ".join(
                    ["{" + str(i) + "," + dquoted(s) + "}" for i, s in fk.base_fields]
                )
                + ", {-1,NULL}\n"
            )
            out.write("};\n\n")
            out.write(
                "static struct fk_field_s descr_fk_fields_out_"
                + fk.name.upper()
                + "[] =\n{\n\t"
            )
            out.write(
                ",".join(
                    ["{" + str(i) + "," + dquoted(s) + "}" for i, s in fk.target_fields]
                )
                + ", {-1,NULL}\n"
            )
            out.write("};\n\n")
        ...
        for t in self.allbeans.values():
            out.write(
                "static struct field_descriptor_s descr_fields_"
                + t.c_name
                + "[] =\n{\n"
            )
            for f in t.fields:
        ...
        for t in self.allbeans.values():
            if len(t.get_fk_names()) > 0:
                out.write(
                    "static gchar * descr_fk_names_"
                    + t.c_name
                    + "[] = { "
                    + ",".join([dquoted(n) for n in t.get_fk_names()])
                    + ", NULL };\n"
                )
            else:
                out.write(
                    "static gchar *descr_fk_names_" + t.c_name + "[] = { NULL };\n"
                )
            out.write("\n")
        ...
        for t in self.allbeans.values():
            out.write(
                "static struct fk_descriptor_s descr_fk_" + t.c_name + "[] =\n{\n"
            )
        ...
        for t in self.allbeans.values():
            out.write(
                "const struct bean_descriptor_s descr_struct_" + t.c_name + " =\n{\n"
            )
        ...
        for t in self.allbeans.values():
            ...
            for f in t.fields:
                ...
                out.write(
                    "\tEXTRA_ASSERT(DESCR(bean) == &descr_struct_" + t.c_name + ");\n"
                )
                out.write(
                    "\t_bean_set_field_value(bean, " + str(f.position) + ", &v);\n"
                )
                out.write("}\n\n")
                if isinstance(f, Text):
                    out.write(
                ...
                if not f.mandatory:
                    ...
                    out.write("\tEXTRA_ASSERT(bean != NULL);\n")
                    out.write(
                        "\tif (_bean_has_field(bean, " + str(f.position) + ")) {\n"
                    )
    ...
    def dump_c_storage(self, out):
        ...
        for t in self.allbeans.values():
            out.write(
                "GError*\n"
                + t.c_name
                + "_load(struct sqlx_sqlite3_s *sq3, const gchar *clause, GVariant"
                " **params,"
            )
            ...
            out.write(
                "\treturn _db_get_bean(&descr_struct_"
                + t.c_name
                + ", sq3, clause, params, cb, u);\n"
            )
            out.write("}\n\n")
...
properties = generator.add_bean(
    ...
    .set_sql_name("properties")
).set_order(5)

contents = generator.add_bean(
    Struct("contents_headers")
    .field(Blob("id"))
    ...
    .set_sql_name("contents")
).set_order(1)

chunks = generator.add_bean(
    Struct("chunks")
    .field(Text("id"))
    .field(Blob("content"))
    .field(Text("position"))
...
generator.add_fk(
    ForeignKey((alias, ("content",), "image"), (contents, ("id",), "aliases"))
)
