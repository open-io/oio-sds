...
class TestDirectoryAPI(BaseTestCase):
    ...
    def _get_properties(self, name, properties=None):
        return self.dir.get_properties(self.account, name, properties=properties)

    def _set_properties(self, name, properties=None):
        return self.dir.set_properties(self.account, name, properties=properties)
    ...
    def test_get_properties(self):
        ...
        self.assertRaises(exc.NotFound, self.dir.get_properties, self.account, name)
    ...
    def test_set_properties(self):
        ...
        self.assertRaises(
            exc.NotFound, self.dir.set_properties, self.account, name, metadata
        )
    ...
    def test_del_properties(self):
        ...
        self.assertRaises(exc.NotFound, self.dir.del_properties, self.account, name, [])
    ...
    def test_list_services(self):
        ...
        echo = "echo"
        self.assertRaises(
            exc.NotFound, self.dir.list, self.account, name, service_type=echo
        )
    ...
    def test_link_rdir_unachievable_min_dist(self):
        ...
        self.assertIn(new_rawx["addr"], all_rawx_keys)
        self.assertRaises(
            exc.VolumeException, disp.rdir._get_rdir_addr, new_rawx["addr"]
        )
    ...
    def _test_link_rdir_fail_to_force(self, side_effects, expected_exc):
        ...
        def _poll(*args, **kwargs):
            ...
            return random.sample(
                all_srvs["rdir"], self.conf.get("directory_replicas", 1)
            )
        ...
        disp._smart_link_rdir = Mock(wraps=disp._smart_link_rdir)
        disp.directory.force = Mock(
            wraps=disp.directory.force, side_effect=side_effects
        )

        rdir_replicas = self.conf.get("directory_replicas", 1)
        # Expect an exception since some assignations will fail
        self.assertRaises(
            expected_exc, disp.assign_all_rawx, replicas=rdir_replicas, max_attempts=1
        )
        ...
        force_calls = [
            call(RDIR_ACCT, rawx["addr"], "rdir", ANY, autocreate=True, replace=ANY)
            for rawx in all_srvs["rawx"]
        ]
