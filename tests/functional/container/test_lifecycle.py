...
class TestLifecycleConformExpirationVersioning(TestLifecycleConformExpiration):
    ...
    def test_delete_marker_2(self):
        ...
        for name in names:
            objects = self.api.object_list(
                self.account, self.container, prefix=name, deleted=True,
                versions=True)
...
class TestLifecycleNonCurrentVersionExpiration(TestLifecycleConform):
    ...
    def setUp(self):
        ...
        self.end_source = """</And>
                    ...
                    <NoncurrentVersionExpiration>
                        <NoncurrentDays>1</NoncurrentDays>
                        <NewerNoncurrentVersions>"""f'{self.newer_non_current_versions}'"""</NewerNoncurrentVersions>
                            </NoncurrentVersionExpiration>
                </Rule>
            </LifecycleConfiguration>"""

        self.not_to_match_versions = []
        self.expected_to_cycle = self.number_of_versions - \
            self.newer_non_current_versions - 1
    ...
    def test_cycle_zero_newer_non_current(self):
        ...
        self.end_source = f'{self.end_source}\
            {self.newer_non_current_versions}'"""</NewerNoncurrentVersions>
