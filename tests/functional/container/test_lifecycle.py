...
class TestLifecycleConformExpiration(CliTestCase, BaseClassLifeCycle):
    ...
    def _get_actions(self, rule):
        ...
        if len(transitions) > 0:
            actions["Transition"] = transitions
        return actions
    ...
    def _exec_rules_via_sql_query(self, source):
        ...
        for el in json_dict["Rules"]:
            actions = self._get_actions(el)
            for act in actions:
                # Force expiration for test
                days_in_sec = 0
                base_sql_query = lc.to_sql_query(el, days_in_sec)
                offset = 0
                action = self.action
                while True:
                    sql_query = (
    ...
    def test_apply_prefix(self):
        source = """{"Rules":
        [{"ID":"rule1","Status":"Enabled","Expiration":{"Days":11},
        "Filter":{"Prefix":"a"}}]
        }"""
    ...
    def test_apply_prefix_and_greater(self):
        ...
        source = (
            """{"Rules":
            [{"ID":"rule1","Status":"Enabled","Expiration":{"Days":11},
            "Filter":{"Prefix":"a", "ObjectSizeGreaterThan":"""
            f"{middle}"
            """}}]}"""
        )
    ...
    def test_apply_prefix_and_lesser(self):
        ...
        source = (
            """{"Rules":
            [{"ID":"rule1","Status":"Enabled","Expiration":{"Days":11},
            "Filter":{"Prefix":"a/", "ObjectSizeLessThan":"""
            f"{middle}"
            """}}]}"""
        )
