...
class TestSharding(TestShardingBase):
    ...
    def test_shard_container(self):
        ...
        shards = self.container_sharding.format_shards(test_shards,
                                                       are_new=True)
        self.container_sharding.replace_shard(self.account, self.cname,
                                              shards, enable=True)
    ...
    def test_add_objects_to_shards(self):
        ...
        test_shards = [{"index": 0, "lower": "", "upper": "1c"},
                       {"index": 1, "lower": "1c", "upper": ""}]
        new_shards = self.container_sharding.format_shards(test_shards,
                                                           are_new=True)
        self.container_sharding.replace_shard(self.account, self.cname,
                                              new_shards, enable=True)
        # check objects
        self._check_total_objects(nb_obj_to_add)
    ...
    def test_successive_shards(self):
        ...
        format_new_shards = self.container_sharding.format_shards(test_shards,
                                                                  are_new=True)
        self.container_sharding.replace_shard(self.account, self.cname,
                                              format_new_shards, enable=True)
        ...
        # reshard
        new_shards = [{"index": 0, "lower": "", "upper": "20"},
                      {"index": 1, "lower": "20", "upper": "30"},
                      {"index": 2, "lower": "30", "upper": ""}]
        format_new_shards = self.container_sharding.format_shards(new_shards,
                                                                  are_new=True)
        self.container_sharding.replace_all_shards(self.account, self.cname,
                                                   format_new_shards,
                                                   enable=True)
    ...
    def test_shard_and_add_delete(self):
        ...
        new_shards = [{"index": 0, "lower": "", "upper": "89"},
                      {"index": 1, "lower": "89", "upper": ""}]
        new_shards = self.container_sharding.format_shards(new_shards,
                                                           are_new=True)
    ...
    def test_threshold(self):
        ...
        for i, threshold in enumerate(thresholds):
            shards = self.container_sharding.find_shards(
                self.account,
                self.cname,
                strategy_params={"strategy": "shard-with-partition",
                                 "threshold": threshold})
