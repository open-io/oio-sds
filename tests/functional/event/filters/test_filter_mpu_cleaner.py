...
class TestFilterMpuCleaner(BaseTestCase):
    ...
    def tearDown(self):
        for container, obj in self.objects_created:
            try:
                self.storage.object_delete(self.account, container, obj)
            except Exception as exc:
                print(f"Failed to delete {self.account}/{container}/{obj}: {exc}")

        for container in (self.container, self.container_segment):
            try:
                self.storage.container_delete(self.account, container)
            except Exception as exc:
                print(f"Failed to delete {self.account}/{container}: {exc}")
        super().tearDown()
    ...
    def _create_manifest_and_parts(self, obj_name, create_manifest=True):
        if create_manifest:
            ...
            self.objects_created.append((self.container, obj_name))
        ...
        for i in range(1, self.nb_parts + 1):
            ...
            self.objects_created.append((self.container_segment, obj))
    ...
    def test_nominal(self):
        ...
        event = self._create_event(
            obj_name, upload_id=self.upload_id, manifest_version=self.manifest_version
        )
        self.mpu_cleaner.process(event, None)

        # Make sure all parts are deleted
        for _ in range(0, self.nb_parts):
            evt = self.wait_for_kafka_event(types=(EventTypes.CONTENT_DELETED,))
            self.assertIsNotNone(evt)
        self._check_nb_objects(self.container_segment, 0)

    def test_expect_no_deletion(self):
        """
        ...
        evt = self.wait_for_kafka_event(types=(EventTypes.CONTENT_DELETED,), timeout=5)
        self.assertIsNone(evt)
        self._check_nb_objects(self.container_segment, self.nb_parts)
