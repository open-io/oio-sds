...
class TestLifecycleAccessLoggerFilter(BaseTestCase):
    ...
    def test_should_log(self):
        ...
        event = Event({"data": {}})
        self.assertFalse(self.filter._should_log(event))
        event = Event({"data": {"has_bucket_logging": False}})
        self.assertFalse(self.filter._should_log(event))
        event = Event({"data": {"has_bucket_logging": True}})
        self.assertFalse(self.filter._should_log(event))
        event = Event({"data": {"has_bucket_logging": True, "bucket": None}})
        self.assertFalse(self.filter._should_log(event))
        event = Event({"data": {"has_bucket_logging": True, "bucket": ""}})
        self.assertFalse(self.filter._should_log(event))
        event = Event({"data": {"has_bucket_logging": False, "bucket": "foobar"}})
        self.assertFalse(self.filter._should_log(event))
        event = Event({"data": {"has_bucket_logging": True, "bucket": "foobar"}})
            ...
            for action, marker, s3action in tests:
                ...
                with patch("logging.Logger.info") as mock_logger:
                    ...
                    mock_logger.assert_called_once_with(
                        "lifecycle_access-foo: - - [30/Sep/2024:07:46:47 +0000] - - "
                        f'- {s3action} test/bar%25F0%259F%2594%25A5 "-" - - - - - - '
                        '"-" "-" - - - - - - - -'
                    )
