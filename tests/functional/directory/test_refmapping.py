...
class TestMeta1RefMapping(BaseTestCase):
    ...
    def _test_move(self, service_type, base=None):
        if base is None:
            base = self._get_cid()
        ...
        if dest_service is None:
            self.skipTest("need at least 2 %s" % service_type)
        expected_services.append(dest_service)

        moved = self.mapping.move(src_service, dest_service, base,
                                  service_type)
        self.mapping.apply(moved)

        data_dir = self.api.directory.list(self.account, self.reference)
        meta2_services = list()
        for d in data_dir['srv']:
            if d['type'] == service_type:
                meta2_services.append(d['host'])
        self.assertListEqual(sorted(expected_services), sorted(meta2_services))
    ...
    def test_move_meta2(self):
        ...
        expected_services = self._test_move('meta2')

        properties = self.api.container_get_properties(
            self.account, self.reference)
        peers = properties['system'].get('sys.peers', None)
    ...
    def test_move_meta2_with_seq(self):
        ...
        expected_services = self._test_move('meta2', base=base)
