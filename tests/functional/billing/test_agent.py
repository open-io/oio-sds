...
class TestBillingAgent(BaseTestCase):
    ...
    def test_send_message(self):
        ...
        def send_and_check(channel_, samples_, expected_counter=0,
                           expected_message_=None):
            ...
            if expected_message_:
                ...
                message = clear_random_value_in_oslo_message(
                    message, '_unique_id')
                message = clear_random_value_in_oslo_message(
                    message, 'message_id')
                message = clear_random_value_in_oslo_message(
        ...
        send_and_check(channel, samples, expected_counter=1,
        ...
        send_and_check(channel, samples, expected_counter=2,
                       expected_message_=expected_message)
    ...
    def test_scan(self):
        ...
        for _ in range(3):
            ...
            self.storage.object_create(
                account_name, bucket_name, obj_name=random_id(8), data='test',
                reqid=reqid)
        ...
        reqid = request_id()
        self.storage.object_create(
            account_name, bucket_name, obj_name=random_id(8), data='',
            reqid=reqid)
        self.wait_for_event('oio-preserved', reqid=reqid,
                            types=(EventTypes.CONTAINER_STATE,))
        ...
        self.storage.container_create(
            account_name, bucket_name,
            system={M2_PROP_BUCKET_NAME: bucket_name}, reqid=reqid)
        self.wait_for_event('oio-preserved', reqid=reqid,
        ...
        channel = FakeChannel()
        with patch("oio.billing.agent.BillingAgent._amqp_connect",
        ...
        self.assertEqual(nb_buckets - nb_sent_buckets,
                         self.agent.ignored)
