...
class TestAutoSharding(BaseTestCase):
    ...
    def test_shrinking_while_flushing(self):
        def _cb(status, _msg):
            self.assertEqual(500, status)
        ...
        self.storage.container_set_properties(
            ...
            None,
            cid=meta2db_neighbour.cid,
            system={M2_PROP_FLUSHING_STATE: str(FLUSHING_STATE_IN_PROGRESS)},
        )
        self.auto_sharding.process(meta2db.env, _cb)
        ...
        def flushing_in_progress_override(meta={}):
            return False

        self.auto_sharding.container_sharding.flushing_in_progress = (
            flushing_in_progress_override
        )
        self.auto_sharding.process(meta2db.env, _cb)
        filter_stats = self.auto_sharding.get_stats()[self.auto_sharding.NAME]
        for key, value in filter_stats.items():
    ...
    def _test_possible_orphan_shard(
        self, sharding_state, in_progress=True, simulate_orphan=True
    ):
        def _cb(status, _msg):
            self.assertEqual(200, status)
        ...
        shard_cid = random.choice(shards)["cid"]
        if simulate_orphan:
            # Simulate orphan shard
            self.auto_sharding._is_orphan = lambda meta2db, reqid, force_master=False: (
                None,
                None,
                True,
            )
    ...
    def test_orphan_shard_with_sharding_in_progress_and_stuck(self):
        self._test_possible_orphan_shard(
            NEW_SHARD_STATE_APPLYING_SAVED_WRITES,
            in_progress=False,
            simulate_orphan=True,
        )
    ...
    def test_orphan_shard_with_sharding_locked_and_stuck(self):
        self._test_possible_orphan_shard(
            EXISTING_SHARD_STATE_LOCKED, in_progress=False, simulate_orphan=True
        )
