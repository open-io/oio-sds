...
class TestUtils(unittest.TestCase):
    def test_obj_range_to_meta_chunk_range(self):
        cases = [
            ...
            dict(
                start=50, end=99, sizes=[50, 30, 20], expected={1: (0, 29), 2: (0, 19)}
            ),
            ...
            dict(
                start=0, end=79, sizes=[50, 30, 20], expected={0: (0, 49), 1: (0, 29)}
            ),
        ...
        for c in cases:
            result = obj_range_to_meta_chunk_range(c["start"], c["end"], c["sizes"])
            self.assertEqual(result, c["expected"])

    def test_deadline_to_timeout(self):
        self.assertRaises(
            DeadlineReached, deadline_to_timeout, monotonic_time() - 0.001, True
        )
        self.assertRaises(DeadlineReached, deadline_to_timeout, monotonic_time(), True)
    ...
    def test_deadline_from_read_timeout(self):
        ...
        self.assertLessEqual(kwargs["deadline"], now + kwargs["read_timeout"] + 0.1)
    ...
    def test_pool_manager_parameters(self):
        ...
        get_pool_manager(pool_connections="5")
        self.assertRaises(ValueError, get_pool_manager, pool_connections="cinq")
        ...
        get_pool_manager(backoff_factor="5", max_retries=5)
        self.assertRaises(
            ValueError, get_pool_manager, backoff_factor="cinq", max_retries=5
        )
