...
class TestContainerLifecycle(unittest.TestCase):
    ...
    def test_NoncurrentCountActionFilter_from_element(self):
        ...
        days = 5
        self.assertRaises(
            ValueError, NoncurrentCountActionFilter.from_element, days, count_elt
        )
    ...
    def test_NoncurrentVersionExpiration_from_element(self):
        ...
        self.assertRaises(ValueError, NoncurrentVersionExpiration.from_element, exp_elt)
        ...
        self.assertRaises(ValueError, NoncurrentVersionTransition.from_element, exp_elt)
    ...
    def test_LifecycleRuleFilter_from_element(self):
        ...
        self.assertRaises(ValueError, LifecycleRuleFilter.from_element, filter_elt)
        ...
        self.assertIsNotNone(filter_)
        self.assertIsNone(filter_.prefix)
        self.assertDictEqual({"key1": "value1", "key2": "value2"}, filter_.tags)
        ...
        self.assertIsNone(filter_.prefix)
        self.assertIsNotNone(filter_.lesser)
        self.assertDictEqual({"key2": "value2"}, filter_.tags)
    ...
    def test_LifecycleRule_from_element(self):
        ...
        # Test valid Prefix outside of Filter tag
        rule_elt = etree.XML(
            """
            <Rule>
                <Prefix>doc/</Prefix>
        ...
        self.assertIsNotNone(rule.filter.prefix)
    ...
    def test_NoncurrentCountActionFilter_to_string(self):
        EXPECTED = (
            "<Noncurrent><NoncurrentDays>1</NoncurrentDays>"
            "<NewerNoncurrentVersions>1</NewerNoncurrentVersions>"
            "</Noncurrent>"
        )
    ...
    def test_Transition_to_string(self):
        EXPECTED = (
            "<Transition><StorageClass>SINGLE</StorageClass>"
    ...
    def test_NoncurrentVersionTransition_to_string(self):
        ...
        days_count = NoncurrentCountActionFilter(10, 0)
        trans = NoncurrentVersionTransition(days_count, "SINGLE", 10, count=None)
        self.assertEqual(EXPECTED, str(trans))
    ...
    def test_LifecycleRuleFilter_to_string(self):
        ...
        EXPECTED = "<Filter><Tag><Key>key</Key><Value>value</Value>" + "</Tag></Filter>"
        ...
        filter_ = LifecycleRuleFilter("documents/", None, None, {"key": "value"})
        self.assertEqual(EXPECTED, str(filter_))

    def test_LifecycleRule_to_string(self):
        EXPECTED = (
            "<Rule><ID>Test</ID><Filter></Filter>"
            + "<Status>Disabled</Status>"
    ...
    def test_DateActionFilter_match(self):
        date_elt = etree.XML("<Date>2006-08-14T02:34:56</Date>")
        date = DateActionFilter.from_element(date_elt)
        self.assertIsNotNone(date)
        ...
        date_elt = etree.XML(
            "<Date>%s</Date>"
            % time.strftime("%Y-%m-%dT%H:%M:%S", time.localtime(time.time() + 86400))
        )
        date = DateActionFilter.from_element(date_elt)
