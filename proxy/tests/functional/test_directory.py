...
TestDirectoryFunctional(unittest.TestCase):
	def
__init__(self, *args, **kwargs):
super(TestDirectoryFunctional, self).
...
self.chars = string.ascii_lowercase + string.ascii_uppercase + string.digits def setUp(self):
super(TestDirectoryFunctional,
	self).setUp()
	...
	return
	''.
join(random.choice(self.chars) for _in range(n))

	self.addr1 = self.basic_addr + '%s' % random.randint(0, 10000)
		self.addr2 = self.basic_addr + '%s' % random.randint(0, 10000)
		self.service =
{
'type': 'echo', 'ns': self.namespace, 'score': 100, 'addr': self.addr1, 'tags':{
'tag.vol': 'test', 'tag.up':True}
}

self.service2 = {
'type': 'echo', 'ns': self.namespace, 'score': 100, 'addr': self.addr2, 'tags':{
'tag.vol': 'test', 'tag.up':True}
}
...
'prop1': self.property3, 'prop2':self.property4}
		}
...
self.addr_RefInvalid = self.address + "/error" self.addr_RefInvalid_type = self.addr_RefInvalid + "/echo" self.addr_RefInvalid_type_action = self.addr_RefInvalid_type + "/action" def tearDown(self):
super(TestDirectoryFunctional,
	self).tearDown()

	for a
in[self.addr_RefSet_type, self.addr_RefSet_type2, self.address_cs, self.addr_RefSet, self.addr_RefSet2]:
try:
	...
	self.session.delete(self.address + "/RefTest")
		...
		def
		test_reference_put_invalid(self):
	resp = self.session.put(self.addr_invalid + "/RefTest")
	self.assertEqual(resp.status_code, 404)

		def
		test_reference_delete(self):
	...
	self.assertEqual(resp.status_code, 404)
		...
		def
		test_reference_delete_invalid(self):
	resp = self.session.delete(self.addr_RefInvalid)
	self.assertEqual(resp.status_code, 404)

		def
		test_reference_head(self):
	...
	self.assertEqual(resp.status_code, 404)
		...
		def
		test_reference_head_invalid(self):
	resp = self.session.head(self.addr_RefInvalid)
	self.assertEqual(resp.status_code, 404)

		def
		test_references_actions_getProperties(self):
...
'action': 'GetProperties', 'args':['prop1', 'prop2']}
	...
	def test_references_actions_GetProperties_Invalid(self):resp =
	self.session.post(self.addr_RefInvalid_type_action, json.dumps( {
'action': 'GetProperties', 'args':None}
		...
		def
		test_references_actions_setProperties(self):
		resp = self.session.post(self.addr_RefSet_action, json.dumps(
		{
'action': 'SetProperties', 'args':{
'prop1': self.property1, 'prop2':self.property2}
		}
...
'action': 'GetProperties', 'args':['prop1', 'prop2']}
	...
	def test_references_actions_setProperties_invalid(self):resp =
	self.session.post(self.addr_RefInvalid_type_action, json.dumps( {
'action': 'SetProperties', 'args':{
'prop1': self.property1, 'prop2':self.property2}
			}
		...
		def
		test_references_actions_delProperties(self):
		resp = self.session.post(self.addr_RefSet_action, json.dumps(
		{
...
'action': 'GetProperties', 'args':['prop1']}
	...
	self.assertEquals(resp,
	{
		...
		def
		test_references_actions_delProperties_invalid(self):
...
assertEqual(resp.status_code, 200)
		...
		def test_service_delete(self):time.sleep(3)
...
#- the meta1 between tests, ghosts services could make them fail
		def
		test_services_actions_link(self):
	time.
sleep(4)
...
"action": "Link", "args":None}
	...
	self.assertEqual(self.addr1, resp)
		...
		def
		test_service_actions_link_again(self):
	self.
	session.
post(self.address_cs + "/action", json.dumps(
		{
...
"action": "Link", "args":None}
	...
	self.assertEqual([self.addr1], resp)
		...
		def
		test_service_actions_renew(self):
	self.
	session.
post(self.address_cs + "/action", json.dumps(
		{
...
"action": "Renew", "args":None}
	...
	self.assertTrue((self.addr1 in resp) and(self.addr2 in resp))
		...
		def
		test_service_action_renew_not_linked(self):
	time.
sleep(4)
...
"action": "Renew", "args":None}
	...
	self.assertEqual([self.addr1], resp)
		...
		def
		test_service_actions_force_no_header(self):
	self.
	session.
post(self.address_cs + "/action", json.dumps(
		{
"action": "Lock", "args":self.service2}
	...
	resp = self.session.post(self.addr_RefSet_type_action2, json.dumps(
		{
"action": "Force", "args":{
"seq": 1, "type": "echo", "host": self.addr2, "args":""}
		}
	...
	self.assertEqual(resp.status_code, 400)
		...
		def
		test_service_actions_force_valid(self):
	self.
	session.
post(self.address_cs + "/action", json.dumps(
		{
