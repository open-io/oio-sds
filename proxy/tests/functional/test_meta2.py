import json
	...
	import requests from tests.functional import load_functest_config class
TestMeta2Functional(unittest.TestCase):
	def
__init__(self, *args, **kwargs):
super(TestMeta2Functional, self).
__init__(*args, **kwargs)
	self.
_load_config()

		def _load_config(self):config = load_functest_config()
...
self.chars = string.ascii_lowercase + string.ascii_uppercase + string.digits self.id_generator = lambda n:''.join(random.choice(self.
		chars) for _in
	range(n))
...
self.bean = self.bean_void self.session.put(self.addr_Ref)
...
for a in[self.addr_m2_ref_path, self.addr_m2_ref_path2,
	self.addr_m2_ref, self.addr_Ref_type, self.addr_Ref]
	:try:self.session.delete(a)
except Exception:pass def prepare_bean(self, i):if i == 1
:
	self.bean = self.session.post(self.addr_m2_ref_path_action, json.dumps(
	{
"action": "Beans", "args":self.bean_void}
	...
	self.bean["size"] = 40
	self.bean["type"] = "chunk" self.prepared_bean = self.bean if i
== 2:
	self.bean2 = self.session.post(self.addr_m2_ref_path_action, json.dumps(
	{
"action": "Beans", "args":self.bean_void}
	)).json()[0]
	self.bean2["id"] = self.bean2["url"]
self.bean2["hash"] = self.hash_rand2 self.bean2["size"] = 80 self.bean2["type"] = "chunk" def post_bean(self):

self.session.put(self.addr_m2_ref_path,
	json.dumps([self.prepared_bean]), headers =
	{
'x-oio-content-meta-hash': self.hash_rand, 'x-oio-content-meta-length':40}

)

		def
		prepare_bean_list(self, i):
	j = 1 while j
<=i:
	self.path_rand = self.id_generator(10)
		self.string_rand = self.id_generator(20)
...
"action": "Beans", "args":self.bean_void}
	...
	self.prepared_bean["type"] = "chunk" self.post_bean()

	self.list_paths.append(self.addr_m2_ref_path)
j += 1 def prepare_bean_listing(self):

self.prepared_bean = self.session.post(self.addr_m2_ref_path_action, json.dumps(
		{
"action": "Beans", "args":self.bean_void}
	...
	self.prepared_bean["hash"] = self.hash_rand
	self.prepared_bean["size"] = 40
	self.prepared_bean["type"] = "chunk" for num1
in xrange(4):
	for num2
in xrange(13):
		self.addr_m2_ref_path = self.addr_m2_ref + '/%d-%02d' % (num1, num2)
			self.addr_m2_ref_path_action = self.addr_m2_ref_path + "/action"
			self.post_bean()
			self.list_paths.append(self.addr_m2_ref_path)
...
in xrange(13):
				self.addr_m2_ref_path = self.addr_m2_ref + '/3-%02d-05' % num
					self.addr_m2_ref_path_action =
					self.addr_m2_ref_path + "/action" self.post_bean()
					self.list_paths.append(self.addr_m2_ref_path)

		def
		delete_bean_list(self):for
	a
	in
	self.
	list_paths:
...
delete(a)
			...
			del self.list_paths
			def prepare_content(self):self.session.put(self.addr_m2_ref)
       self.
prepare_bean(1)
	self.
post_bean()

		def prepare_properties(self):self.prepare_content()
    self.session.
post(self.addr_m2_ref_path_action, json.dumps(
		{
"action": "SetProperties", "args":{
"prop1": self.prop_rand, "prop2":self.prop_rand2}
		}

	))
#Containers tests
		def test_containers_put(self):resp = self.session.put(self.addr_m2_ref)
	self.assertEqual(resp.status_code, 204)
	resp = self.session.head(self.addr_m2_ref)
	self.assertEqual(resp.status_code, 204)

		def
		test_container_head(self):
	self.
	session.
put(self.addr_m2_ref)
	resp = self.session.head(self.addr_m2_ref)
	self.assertEqual(resp.status_code, 204)

		def
		test_container_head_ref_no_link(self):
	resp = self.session.head(self.addr_m2_alone_ref)
	self.assertEqual(resp.status_code, 404)

		def
		test_container_head_ref_link(self):
	resp = self.session.head(self.addr_m2_ref)
	self.assertEqual(resp.status_code, 404)
...
prepare_content()
	...
	self.assertEqual(type(resp.json()), dict)
		...
		def
		test_containers_get_marker(self):
	self.
	session.
put(self.addr_m2_ref)
	self.
...
before_mark =[bean["name"] for bean in
	self.session.get(self.addr_m2_ref, params =
		{
'marker_end':'W'}
...
in after_mark:
			...
			def test_containers_get_prefix(self):self.session.put(self.
	addr_m2_ref)
    self.
prepare_bean_list(2)

marker = self.session.get(self.addr_m2_ref).json()['objects'][0]['name'][0:3]
...
self.assertTrue(name[0:3] == marker)
		...
		def test_containers_get_delimiter(self):self.session.put(self.
	addr_m2_ref)
    self.
prepare_bean_list(5)
...
marker = self.session.get(self.addr_m2_ref).json()['objects'][0]['name'][2:3]
...
not in delimit_mark:
				...
				def test_containers_get_max(self):self.session.put(self.
	addr_m2_ref)
...
'max':	3}
	...
	def test_container_get_ref_link(self):resp =
	self.session.get(self.addr_m2_ref)
	self.assertEqual(resp.status_code, 404)

		def
		test_container_get_listing(self):
	self.
	session.
put(self.addr_m2_ref)
	self.
prepare_bean_listing()

listing = self.session.get(self.addr_m2_ref, params =
	{
'max':10}
...
'max': 10, 'prefix': '2', 'delimiter':'-'}
	...
	listing = self.session.get(self.addr_m2_ref, params =
	{
'max': 6, 'prefix': '2-', 'marker': '2-04', 'delimiter':'-'}
	...
	listing = self.session.get(self.addr_m2_ref, params =
	{
'max': 10, 'prefix': '3-', 'marker': '3-05', 'delimiter':'-'}
...
#Failed get_list tests
		def
		test_container_get_listing6(self):
	self.
	session.
put(self.addr_m2_ref)
	self.
...
put(self.addr_m2_ref)
	...
	self.assertEqual(resp.status_code, 404)
		...
		def
		test_container_delete_ref_no_link(self):
	resp = self.session.delete(self.addr_m2_alone_ref)
	self.assertEqual(resp.status_code, 404)

		def
		test_container_delete_ref_link(self):
	resp = self.session.delete(self.addr_m2_ref)
	self.assertEqual(resp.status_code, 404)
#Containers Actions tests
		def
		test_containers_actions_touch(self):
	self.
	session.
put(self.addr_m2_ref)
...
#def test_containers_actions_purge(self):  # ignored
		def
		test_containers_actions_setStoragePolicy(self):
	self.
	session.
put(self.addr_m2_ref)
	self.
...
"action": "SetStoragePolicy", "args":"TWOCOPIES"}
	...
	self.assertEqual(resp.status_code, 200)
	self.session.put(self.addr_m2_ref_path, json.dumps([self.bean]), headers =
	{
'x-oio-content-meta-hash': self.hash_rand, 'x-oio-content-meta-length':40}
	...
	self.assertEqual(resp, "TWOCOPIES")
		...
		def
		test_containers_actions_setStoragePolicy_ref_no_link(self):
...
"action": "SetStoragePolicy", "args":"TWOCOPIES"}
	...
	self.assertEqual(resp.status_code, 404)
		...
		def
		test_containers_actions_setStoragePolicy_ref_link(self):
	resp = self.session.post(self.addr_m2_ref_action, json.dumps(
		{
"action": "SetStoragePolicy", "args":"TWOCOPIES"}
	...
	self.assertEqual(resp.status_code, 404)
		...
		def
		test_containers_actions_setStoragePolicy_wrong(self):
	self.
	session.
put(self.addr_m2_ref)
	self.
prepare_bean(1)
self.session.
post(self.addr_m2_ref_action, json.dumps(
		{
...
'x-oio-content-meta-hash': self.hash_rand, 'x-oio-content-meta-length':40}
	...
	self.assertEqual(resp, "none")
		...
		def
		test_containers_actions_getProperties(self):
	self.
	session.
put(self.addr_m2_ref)
	resp = self.session.post(self.addr_m2_ref_action, json.dumps(
...
"action": "GetProperties", "args":None}
	...
	self.assertEqual(type(resp.json()), dict)
		...
		def
		test_containers_actions_getProperties_ref_no_link(self):
	resp = self.session.post(self.addr_m2_alone_ref_action, json.dumps(
		{
"action": "GetProperties", "args":None}
	...
	self.assertEqual(resp.status_code, 404)
		...
		def
		test_containers_actions_getProperties_ref_link(self):
	resp = self.session.post(self.addr_m2_ref_action, json.dumps(
		{
"action": "GetProperties", "args":None}
	...
	self.assertEqual(resp.status_code, 404)
		...
		def
		test_containers_actions_setProperties(self):
	self.
	session.
put(self.addr_m2_ref)
	resp = self.session.post(self.addr_m2_ref_action, json.dumps(
...
"action": "GetProperties", "args":None}
	...
	self.assertEqual(resp, self.prop)
		...
		def
		test_containers_actions_setProperties_wrong(self):
#to be improved
	self.
	session.
put(self.addr_m2_ref)
...
"error":	self.prop}
	...
	self.assertFalse(resp.status_code == 500)
		...
		def
		test_containers_actions_setProperties_ref_no_link(self):
	resp = self.session.post(self.addr_m2_alone_ref_action, json.dumps(
		{
"action": "SetProperties", "args":{
"sys.user.name":self.prop}
	...
	self.assertEqual(resp.status_code, 404)
		...
		def
		test_containers_actions_setProperties_ref_link(self):
	resp = self.session.post(self.addr_m2_ref_action, json.dumps(
		{
"action": "SetProperties", "args":{
"sys.user.name":self.prop}
	...
	self.assertEqual(resp.status_code, 404)
		...
		def
		test_containers_actions_DelProperties(self):
	self.
	session.
put(self.addr_m2_ref)
	resp = self.session.post(self.addr_m2_ref_action, json.dumps(
...
"action": "GetProperties", "args":None}
	...
	self.assertFalse("sys.user.name" in resp.keys())
		...
		def
		test_containers_actions_DelProperties_no_link(self):
	resp = self.session.post(self.addr_m2_alone_ref_action, json.dumps(
		{
"action": "DelProperties", "args":["sys.user.name"]}
	...
	self.assertEqual(resp.status_code, 404)
		...
		def
		test_containers_actions_DelProperties_link(self):
	resp = self.session.post(self.addr_m2_ref_action, json.dumps(
		{
"action": "DelProperties", "args":["sys.user.name"]}
	...
	self.assertEqual(resp.status_code, 404)
		...
		def
		test_containers_actions_rawInsert(self):
#to be improved
	self.
	session.
put(self.addr_m2_ref)
...
"action": "RawInsert", "args":[self.bean]}
	...
	self.assertEqual(resp.status_code, 204)
		...
		def
		test_containers_actions_rawDelete(self):
#to be improved
	self.
prepare_content()
	resp = self.session.post(self.addr_m2_ref_action, json.dumps(
...
"action": "RawDelete", "args":[self.bean]}
	...
	self.assertEqual(resp.status_code, 204)
		...
		def
		test_containers_actions_rawUpdate(self):
	self.
prepare_content()
	self.
prepare_bean(2)
	resp = self.session.post(self.addr_m2_ref_action, json.dumps(
		{
"action": "RawUpdate", "args":{
"new": [self.bean2], "old":[self.bean]}
		}
	...
	self.assertEqual(resp[0]["hash"], self.bean2["hash"].upper())
		...
		def
		test_containers_actions_rawUpdate_ref_link(self):
	self.
prepare_content()
self.session.
post(self.addr_alone_ref_type_action, json.dumps(
		{
"action": "Link", "args":None}
...
prepare_content()
	...
	self.assertEqual(resp.status_code, 204)
		...
		def
		test_content_head_void_container(self):
	self.
	session.
put(self.addr_m2_ref)
	resp = self.session.head(self.addr_m2_ref_path)
...
prepare_content()
	...
	self.assertEqual(resp.json()[0]["hash"], self.hash_rand.upper())
		...
		def
		test_contents_get_void_container(self):
	self.
	session.
put(self.addr_m2_ref)
	resp = self.session.get(self.addr_m2_ref_path)
...
'x-oio-content-meta-hash': self.hash_rand, 'x-oio-content-meta-length':40}
	...
	self.assertEqual(resp, self.hash_rand.upper())
		...
		def
		test_contents_put_ref_link(self):
	self.
	session.
put(self.addr_m2_ref)
	self.
...
'x-oio-content-meta-hash': self.hash_rand, 'x-oio-content-meta-length':40}
	...
	self.assertEqual(resp.status_code, 404)
		...
		def
		test_contents_put_invalid_headers(self):
	self.
	session.
put(self.addr_m2_ref)
	self.
prepare_bean(1)
raisedException = False try:
resp = self.session.put(self.addr_m2_ref_path, json.dumps([self.bean]),
	headers =
	{
'x-oio-content-meta-hash': 'error', 'x-oio-content-meta-length':40}

)
		except
			Exception:
			raisedException = True pass self.assertFalse(raisedException)
...
self.session.put(self.addr_m2_ref)
resp = self.session.post(self.addr_m2_ref_path_action, json.dumps({ "action": "Beans", "args":self.bean_void
			}
	))
	self.assertEqual(resp.status_code, 200)
	for label
in["url", "pos", "size", "hash"]:
	self.assertTrue(label in resp.json()[0].keys())

def test_contents_actions_beans_wrong_arg(self):

	self.session.put(self.addr_m2_ref)
resp = self.session.post(self.addr_m2_ref_path_action, json.dumps({ "action": "Beans", "args": {'error':'error'}
			}
...
def test_contents_actions_Spare(self):
#to be improved

self.session.put(self.addr_m2_ref)
resp = self.session.post(self.addr_m2_ref_path_action, json.dumps({ "action": "Spare", "args": {"size": 40, "notin":{}
, "broken":{}
			}
		}
...
self.prepare_content()
resp = self.session.post(self.addr_m2_ref_path_action, json.dumps({ "action": "Touch", "args":self.direct_path
			}
	))
	self.assertTrue(resp.status_code == 204)
...
self.prepare_content()
resp = self.session.post(self.addr_m2_ref_path_action, json.dumps({ "action": "SetStoragePolicy", "args":'TWOCOPIES' }
...
self.prepare_content()
resp = self.session.post(self.addr_m2_ref_path_action, json.dumps({ "action": "SetStoragePolicy", "args":'error' }
...
resp = self.session.post(self.addr_m2_ref_path_action, json.dumps({ "action": "GetProperties", "args":None }
...
self.prepare_content()
resp = self.session.post(self.addr_m2_ref_path_action, json.dumps({ "action":"SetProperties",
...
self.prepare_properties()
resp = self.session.post(self.addr_m2_ref_path_action, json.dumps({ "action": "DelProperties", "args":['prop1'] }
