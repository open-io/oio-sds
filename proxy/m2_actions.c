...
static oio_sweight_t
_patch_score(const oio_sweight_t score,
		...
		const oio_location_t location)
{
	...
	const oio_sweight_t pre_score = srv.score;
#endif
	srv.score = _patch_score(srv.score, srv.location, location);
#ifdef HAVE_EXTRA_DEBUG
	if (pre_score != srv.score) {
		GRID_TRACE("Get score changed for %s: %d -> %d", chunk_id, pre_score, srv.score);
	}
#endif
...
static enum http_rc_e
action_m2_container_lifecycle_prepare_events(struct req_args_s *args, struct json_object *j UNUSED)
{
	GError *err = NULL;
	gchar *offset = NULL;
	oio_ext_allow_long_timeout(TRUE);
	...
	PACKER_VOID(_pack) {
		return m2v2_remote_pack_PREPARE_LIFECYCLE(args->url, args->rq->body,
				DL());
	};
...
// CONTENT{{
// POST /v3.0/{NS}/container/lifecycle/prepare?cid={cid}&service_id={service_id}
// ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
// Prepare logic to send events
//
// .. code-block:: http
//
//    POST /v3.0/OPENIO/container/lifecycle/prepare?cid={cid}&service_id={service_id} HTTP/1.1
//    Host: 127.0.0.1:6000
//    User-Agent: curl/7.47.0
//    Accept: */*
//
...
// }}CONTENT
enum http_rc_e
action_container_lifecycle_prepare_events(struct req_args_s *args)
{
	return rest_action(args, action_m2_container_lifecycle_prepare_events);
}
