...
void
service_info_merge_tags (struct service_info_s *dst, struct service_info_s *src)
{
	for (guint i=0; i<src->tags->len ;i++) {
		struct service_tag_s *t = src->tags->pdata[i];
		struct service_tag_s *t0 = service_info_ensure_tag (dst->tags, t->name);
		...
		struct service_info_s **out, gboolean permissive)
{
	...
	if (ns && !json_object_is_type(ns, json_type_null))
		metautils_strlcpy_physical_ns(si->ns_name, json_object_get_string(ns), sizeof(si->ns_name));
	if (type && !json_object_is_type(type, json_type_null))
	...
	if (tags) {
		...
		json_object_object_foreach(tags,key,val) {
			...
			if (tag) g_ptr_array_add(si->tags, tag);
		}
	}
	*out = si;
	return NULL;
...
GError*
service_info_load_json(const gchar *encoded, struct service_info_s **out, gboolean permissive)
{
