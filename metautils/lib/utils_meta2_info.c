...
#ifndef LOG_DOMAIN
#define LOG_DOMAIN "metautils.meta2_info"
#endif
...
gint
meta2_info_to_string(const meta2_info_t * src, gchar * dst, gsize dstSize)
{
	gsize offset = 0;
...
void
meta2_info_print_all(const gchar * domain, const gchar * header, GSList * list)
{
	gchar str_pi[2048];

	void func_debug_meta2info(gpointer d, gpointer u)
	{
		(void) u;

		if (d) {
			memset(str_pi, 0x00, sizeof(str_pi));
			meta2_info_to_string((meta2_info_t *) d, str_pi, sizeof(str_pi) - 1);
			TRACE_DOMAIN(domain, "%s%s", header, str_pi);
		}
	}

	g_slist_foreach(list, func_debug_meta2info, NULL);
}

void
meta2_info_gclean(gpointer d, gpointer u)
{
	(void) u;
...
gint
meta2_info_sort_by_score(gconstpointer a, gconstpointer b)
{
...
gint
meta2_info_comp(gconstpointer a, gconstpointer b)
{
	meta2_info_t *m1 = (meta2_info_t *) a;
	meta2_info_t *m2 = (meta2_info_t *) b;
