...
class RawxDecommissionTask(XcuteTask):
    ...
    def process(self, task_id, task_payload, reqid=None):
        ...
        if all(item in task_payload for item in ["container_id", "content_id"]):
            if container_id != task_payload["container_id"]:
                raise ValueError(
                    "Mismatch container ID: %s != %s",
                    container_id,
                    task_payload["container_id"],
                )
            if content_id != task_payload["content_id"]:
                raise ValueError(
                    "Mismatch content ID: %s != %s",
                    content_id,
                    task_payload["content_id"],
                )
...
class RawxDecommissionJob(XcuteUsageTargetJob):
    ...
    def get_tasks(self, job_params, marker=None, reqid=None):
        ...
        for chunk_id in chunk_info:
            task_id = "|".join((job_params["service_id"], chunk_id))
            yield task_id, {
                "chunk_id": chunk_id,
            }
