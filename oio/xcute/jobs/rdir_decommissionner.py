    ...
    def process(self, task_id, task_payload, reqid=None):
        ...
        if self.dry_run:
            self.logger.debug(
                ...
                reqid, hosted_service_id, hosted_service_type)
            return {'skipped_bases': 1}

        dispatcher = RdirDispatcher(self.conf, logger=self.logger)
        old_rdirs = dispatcher.rdir._get_rdir_addr(hosted_service_id,
                                                   reqid=reqid)
        self.logger.debug('[reqid=%s] Reassigning DB of %s out of %s',
                          reqid, hosted_service_id, self.service_id)
        ...
        new_rdirs = {(x.get('tag.service_id') or x['addr'])
                     for x in all_services[0]['rdir']} - set(old_rdirs)
        copy_func = {
            'meta2': dispatcher.rdir.meta2_copy_vol,
            'rawx': dispatcher.rdir.chunk_copy_vol,
        }
        self.logger.debug('[reqid=%s] Copying DB of %s to %s',
                          reqid, hosted_service_id, new_rdirs)
        # Let this deal with available sources automatically
        copy_func[hosted_service_type](
            hosted_service_id,
            dests=list(new_rdirs),
            reqid=reqid)
...
class RdirDecommissionJob(XcuteRdirJob):
    ...
    def sanitize_params(cls, job_params):
        sanitized_job_params, _ = super(
            RdirDecommissionJob, cls).sanitize_params(job_params)
        ...
        sanitized_job_params['min_dist'] = int_value(
            job_params.get('min_dist'),
            cls.DEFAULT_MIN_DIST)

        sanitized_job_params['replicas'] = int_value(
            job_params.get('replicas'),
            cls.DEFAULT_REPLICAS)
    ...
    def get_tasks(self, job_params, marker=None):
        typed_iters = [self.get_tasks_for_type(job_params, type_)
