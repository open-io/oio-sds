    ...
    def _update_new_shard(self, new_shard, buffer_size=1000, **kwargs):
        ...
        while True:
            ...
            try:
                ...
                if not self.running and self.queue_is_empty and not buffer:
                    return
...
class ContainerSharding(ProxyClient):
    ...
    def _find_shards_with_partition(self, shard, incomplete_shard=None,
        ...
        if strategy_params:
            ...
            threshold = strategy_params.get('threshold')
        if not partition:
            partition = self.DEFAULT_PARTITION
        if threshold is None:
            threshold = self.DEFAULT_SHARD_SIZE
    ...
    def _find_shards_with_size(self, shard, incomplete_shard=None,
        ...
        if strategy_params:
            shard_size = strategy_params.get('shard_size')
        if not shard_size:
    ...
    def _shard_container_by_dichotomy(self, root_account, root_container,
                                      ...
                                      max_new_shards_by_sharding=2, **kwargs):
        new_shards_size = len(new_shards)
        if new_shards_size <= max_new_shards_by_sharding:
        ...
        start_index = 0
        end_index = 0
        for i in range(max_new_shards_by_sharding):
            end_index += new_shards_size // max_new_shards_by_sharding
            if i < new_shards_size % max_new_shards_by_sharding:
                end_index += 1
        ...
        for i in range(max_new_shards_by_sharding):
            ...
            self._shard_container_by_dichotomy(
                ...
                max_new_shards_by_sharding=max_new_shards_by_sharding,
                **kwargs)
