...
class ContainerLifecycle(object):
    ...
    def load_json(self, json_str):
        ...
        try:
            ...
            self.logger.warning(f"Failed to decode json conf {json_str}")
    ...
    def to_sql_query(self, rule_filter, formated_time=None, **kwargs):
        ...
        if rule.prefix is not None:
            _prefix_cond = " WHERE ( al.alias LIKE '" f"{rule.prefix}" "%'"
...
class RuleFilter(object):
    def __init__(self, rule_filter):
        ...
        elif rule_filter.get("Filter", {}).get("Tag") is not None:
            self.tags = [rule_filter.get("Filter", {}).get("Tag")]
