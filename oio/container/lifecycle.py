    ...
    def from_element(cls, rule_elt, **kwargs):
        ...
        for transition_elt in rule_elt.findall(
            ...
            if action_filter_type is None:
                ...
                raise ValueError(
                    "'NoncurrentDays' and 'NewerNoncurrentVersions' in "
                    "the same Rule")
            transitions.append(transition)
        if transitions:
...
class LifecycleRuleFilter(object):
    ...
    def to_sql_query(self, formated_days=None, date=None, non_current=False,
                     versioned=False, expired_delete_marker=None,
                     **kwargs):
        # Beginning of query will be force by meta2 code to avoid
        # update or delete queries

        if versioned:
            _query = (' al.alias, al.version, al.content, al.deleted,'
                      'al.ctime, al.mtime, count(*) as nb_versions,'
                      'max(al.version) FROM versioned_view AS al')
        else:
        ...
        if not skip_size_and_tag:
            ...
            if len(self.tags) > 0:
                _base_tag = (' INNER JOIN properties pr2 ON al.alias ='
                ...
                for k, v in self.tags.items():
                    ...
                    value_elt.text = v
                    tag_elt.append(value_elt)
                    _tag_key_cond = (' AND CAST(pr2.value as nvarchar(10000))'
    ...
    def create_noncurrent_view(self, formated_time=None, **kwargs):
        ...
        if formated_time is not None:
            _time_cond = f' ((al.mtime + {formated_time}) < (CAST '\
                f'(strftime(\'%s\', \'now\') AS INTEGER )))'
            _query = f'{_query}{_time_cond}'
        # close WHERE clause
        if self.prefix is not None or formated_time is not None:
    ...
    def create_common_views(self, view_name, formated_time=None, deleted=None,
                            **kwargs):
        if not view_name:
            raise ValueError('Lifecycle views, empty view name!')
        # CREATE forced on meta2 side
        _query = (f'VIEW {view_name} AS SELECT *, '
    ...
    def noncurrent_query(self):
        ...
        query = (
            ' nv.alias, nv.version, nv.content,  nv.deleted,'
            ' nv.ctime, nv.mtime, cv.nb_versions,'
            ' (SELECT COUNT(*) FROM noncurrent_view WHERE alias=nv.alias)'
            ' AS count FROM noncurrent_view AS nv'
    ...
    def from_element(cls, delete_marker, **kwargs):
        try:
            ...
            if delete_marker_text not in ('false', 'true'):
                raise ValueError(
                    "The ExpiredObjectDeleteMarker must be a boolean")
...
class NoncurrentCountActionFilter(LifecycleActionFilter):
    ...
    def from_element(cls, days, count_elt, **kwargs):
        if count_elt is not None:
            try:
                ...
                raise ValueError("The count must be greater "
    ...
    def match(self, obj_meta, now=None, **kwargs):
        ...
        if float(obj_meta['mtime']) + self.days * 86400 < now:
            if self.lifecycle is None or self.lifecycle.processed_versions is\
               None:
            ...
            processed = self.lifecycle.processed_versions.nb_processed(
                obj_meta)
    ...
    def from_element(cls, expiration_elt, **kwargs):
        ...
        try:
            expired_delete_marker_elt = expiration_elt.findall(
                tag('ExpiredObjectDeleteMarker'), nsmap)[-1]
        except IndexError:
            expired_delete_marker_elt = None

        if days_elt is None and date_elt is None and \
           expired_delete_marker_elt is None:
            raise ValueError(
            ...
            raise ValueError("'Days' and 'ExpiredObjectDeleteMarker' in "
                             "same Expirationaction action")
        elif days_elt is not None and expired_delete_marker_elt is not None:
            raise ValueError("'Days' and 'ExpiredObjectDeleteMarker' in "
                             "same Expiration action")
...
class NoncurrentVersionExpiration(Expiration):
    ...
    def from_element(cls, expiration_elt, **kwargs):
        ...
        nsmap = expiration_elt.nsmap
        try:
            non_current_days_elt = expiration_elt.findall(
                tag('NoncurrentDays'), nsmap)[-1]
        except IndexError:
            raise ValueError("Missing 'NoncurrentDays '"
                             "element in NoncurrentVersionExpiration action")
        try:
            newer_non_current_el = expiration_elt.findall(
                tag('NewerNoncurrentVersions'), nsmap)[-1]
        except IndexError:
            newer_non_current_el = None

        non_current_days = int(non_current_days_elt.text or '')
        if non_current_days <= 0:
            raise ValueError("'NoncurrentDays' for NoncurrentVersionExpiration"
        ...
        return cls(action_filter, non_current_days,
                   newer_non_current_versions, **kwargs)
...
class NoncurrentVersionTransition(Transition):
    ...
    def from_element(cls, transition_elt, **kwargs):
        ...
        return cls(action_filter, policy, non_current_days,
                   newer_non_current_versions, **kwargs)
