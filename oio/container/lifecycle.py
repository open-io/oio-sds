...
class ProcessedVersions(object):
    ...
    def is_already_processed(self, obj_meta, **kwargs):
        ...
        """
        return (
            obj_meta["name"] == self.name and int(obj_meta["version"]) in self.versions
        )
...
class ContainerLifecycle(object):
    ...
    def load_xml(self, xml_str):
        ...
        if tree.tag != tag("LifecycleConfiguration"):
            raise ValueError(
                "Expected 'LifecycleConfiguration' as root tag, got '%s'" % tree.tag
            )
    ...
    def _to_element_tree(self, **kwargs):
        lifecycle_elt = etree.Element("LifecycleConfiguration", nsmap={None: XMLNS_S3})
    ...
    def save(self, xml_str=None):
        ...
        """
        if not self.rules:
            raise ValueError("You must call `load_xml()` parameter before saving")
        self.api.container_set_properties(
            self.account, self.container, properties={LIFECYCLE_PROPERTY_KEY: str(self)}
        )
    ...
    def order_rules(self, versioned=False):
        ...
        abort_incomplete_mpu_rules = dict()
        ...
        for rule in self.rules:
            for act in rule.actions:
                if isinstance(act, NoncurrentVersionExpiration) or isinstance(
                    ...
                    abort_incomplete_mpu_rules[id_incomplete_mpu] = (rule, act)
                    id_incomplete_mpu = id_incomplete_mpu + 1
        ...
        sorted_noncurrent_rules = dict(
            sorted(
                ...
                key=lambda item: (item[1][1].filter.days,),
                reverse=False,
            )
        )

        sorted_abort_incomplete_mpu_rules = dict(
            sorted(
                abort_incomplete_mpu_rules.items(),
                key=lambda item: (
                    item[1][1].filter.days,
        ...
        ordered_rules = [
            ...
            delete_marker_rules,
            sorted_abort_incomplete_mpu_rules,
        ]
        ...
        for obj_meta in depaginate(
            ...
            **kwargs,
            ...
            try:
                ...
                if (
                    self.processed_versions is not None
                    and self.processed_versions.is_already_processed(obj_meta, **kwargs)
                ):
                    continue
    ...
    def execute(self, use_processed_versions=True, **kwargs):
        ...
        if self._is_delete_incomplete_action():
            ...
            try:
                ...
                self.logger.warning("AbortIncompleteMultiPartUpload can't apply")
                return False
...
class LifecycleRule(object):
    ...
    def from_element(cls, rule_elt, **kwargs):
        ...
        try:
            ...
            try:
                filter_ = LifecycleRuleFilter.from_element(rule_elt)
            except IndexError:
        ...
        try:
            abortIncompleteMpu = AbortIncompleteMultipartUpload.from_element(
                rule_elt.findall(tag("AbortIncompleteMultipartUpload"), nsmap)[-1],
                **kwargs,
            )
        ...
        if transitions:
            if action_filter_type == DateActionFilter:
                transitions = sorted(
                    transitions,
                    key=lambda transition: transition.filter.date,
                    reverse=True,
                )
            ...
            if expiration:
                if type(expiration.filter) is DeletedMarkerActionFilter:
        ...
        if not actions:
            raise ValueError("At least one action needs to be specified in a Rule")
    ...
    def apply(self, obj_meta, **kwargs):
        ...
        if self.enabled and self.match(obj_meta):
            for action in self.actions:
                if type(action) is AbortIncompleteMultipartUpload and not (
                    obj_meta["container"]
                ).endswith("+segments"):
                    continue

                if type(action) is not AbortIncompleteMultipartUpload and (
                    obj_meta["container"]
                ).endswith("+segments"):
                    continue
                try:
...
class LifecycleRuleFilter(object):
    ...
    def from_element(cls, filter_elt, **kwargs):
        ...
        def find_tag(element, tag_name):
            try:
                tag_value = int(element.findall(tag(tag_name), nsmap)[-1].text)
                if tag_value < 0:
                    raise ValueError("The %s size must be a positive integer", tag_name)
            except IndexError:
                tag_value = None
            return tag_value
        ...
        try:
            ...
            try:
                k, v = cls._tag_from_element(filter_elt.findall(tag("Tag"), nsmap)[-1])
                tags = {k: v}
            except IndexError:
                tags = {}
            filter_elements = (prefix, greater, lesser)
        ...
        if self.tags:
            ...
            if tagging_xml is not None:
                ...
                if tagging_elt.tag != expected_tag:
                    raise ValueError(
                        "Expected 'Tagging' as root tag, got '%s'" % tagging_elt.tag
                    )
...
class DaysActionFilter(LifecycleActionFilter):
    ...
    def from_element(cls, days_elt, **kwargs):
        try:
            ...
            raise ValueError("The days must be a positive integer")
...
class LifecycleAction(LifecycleActionFilter):
    ...
    def match(self, obj_meta, **kwargs):
        return self.lifecycle.is_current_version(
            obj_meta, **kwargs
...
class AbortIncompleteMultipartUpload(LifecycleAction):
    ...
    def from_element(cls, abortincompletempu_elt, **kwargs):
        ...
        try:
            days_elt = abortincompletempu_elt.findall(
                tag("DaysAfterInitiation"), nsmap
            )[-1]
        except IndexError:
            raise ValueError("No DaysAfterInitiation field")

        action_filter = DaysAfterInitiationActionFilter.from_element(days_elt, **kwargs)
    ...
    def match(self, obj_meta, **kwargs):
        status = super(AbortIncompleteMultipartUpload, self).match(obj_meta, **kwargs)
        if status:
            current_obj = self.lifecycle.api.object_get_properties(
                self.lifecycle.account, obj_meta["container"], obj_meta["name"]
            )
            is_incomplete = current_obj.get("properties", {}).get(
                "x-object-sysmeta-s3api-has-content-type", None
            )
            if is_incomplete:
                return True
        else:
    ...
    def apply(self, obj_meta, version=None, **kwargs):
        if self.match(obj_meta, **kwargs):
            list_parts = self.lifecycle.api.list_objects(
                self.lifecycle.account, obj_meta["container"], prefix=obj_meta["name"]
            )
            res = None
            for part in list_parts:
                res = self.lifecycle.api.object_delete(
                    self.lifecycle.account, obj_meta["container"], part
                )
...
class Transition(LifecycleAction):
    ...
    def from_element(cls, transition_elt, **kwargs):
        ...
        try:
            policy = transition_elt.findall(tag(cls.STORAGE_POLICY_XML_TAG), nsmap)[
                -1
            ].text
            if policy is None:
                raise ValueError(
                    "Missing value for '%s' element" % cls.STORAGE_POLICY_XML_TAG
                )
        except IndexError:
            raise ValueError(
                "Missing '%s' element in Transition action" % cls.STORAGE_POLICY_XML_TAG
            )
        ...
        if days_elt is None and date_elt is None:
            raise ValueError("Missing 'Days' or 'Date' element in Transition action")
        elif days_elt is not None and date_elt is not None:
            raise ValueError("'Days' and 'Date' in same Transition action")
