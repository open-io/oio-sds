...
class Tool(object):
    ...
    def __init__(self, conf, beanstalkd_addr=None, logger=None, watchdog=None):
        ...
        self.retry_delay = int_value(
            self.conf.get("retry_delay"), self.DEFAULT_RETRY_DELAY
        )
    ...
    def _fetch_items_with_beanstalkd_reply_from_beanstalkd(self):
        ...
        return self.beanstalkd.fetch_jobs(
            self._item_with_beanstalkd_reply_from_task_event, reserve_timeout=2
        )
    ...
    def _update_total_counters(self):
        ...
        return items_processed, self.total_items_processed, errors, self.total_errors
        ...
        try:
            if (
                ...
                self.beanstalkd_reply = BeanstalkdSender(
                    beanstalkd_reply["addr"], beanstalkd_reply["tube"], self.logger
                )
...
class _LocalDispatcher(_Dispatcher):
    ...
    def __init__(self, conf, tool):
        ...
        concurrency = int_value(
            self.conf.get("concurrency"), self.tool.DEFAULT_CONCURRENCY
        )
        self.max_items_per_second = int_value(
            self.conf.get("items_per_second"), self.tool.DEFAULT_ITEM_PER_SECOND
        )
        ...
        for _ in range(concurrency):
            worker = self.tool.create_worker(self.queue_workers, self.queue_reply)
    ...
    def _fill_queue(self):
        ...
        try:
            items_with_beanstalkd_reply = self.tool.fetch_items_with_beanstalkd_reply()
            for item_with_beanstalkd_reply in items_with_beanstalkd_reply:
                items_run_time = ratelimit(items_run_time, self.max_items_per_second)
...
class _DistributedDispatcher(_Dispatcher):
    ...
    def __init__(self, conf, tool):
        ...
        for beanstalkd in locate_tube(all_available_beanstalkd.values(), workers_tube):
            ...
            self.logger.info(
                ...
                beanstalkd_worker.addr,
                beanstalkd_worker.tube,
            )
        ...
        try:
            ...
            for local_service in local_services:
                if local_service["type"] != "beanstalkd":
                    continue
                beanstalkd = all_available_beanstalkd.get(local_service["addr"])
            ...
            self.logger.warning("ERROR when searching for beanstalkd locally: %s", exc)
        if not beanstalkd_reply:
            ...
            try:
                ...
                self.logger.warning("ERROR when searching for beanstalkd: %s", exc)
        ...
        tubes = Beanstalk.from_url("beanstalk://" + beanstalkd_reply_addr).tubes()
        ...
        self.logger.info(
            ...
            self.beanstalkd_reply.addr,
            self.beanstalkd_reply.tube,
        )
    ...
    def run(self):
        ...
        thread = threading.Thread(target=self._distribute_events, args=[reply_loc])
        thread.start()
        ...
        try:
            ...
            self.logger.error(
                "No response for %d seconds", DISTRIBUTED_DISPATCHER_TIMEOUT
            )
            self.tool.success = False
        except Exception:  # pylint: disable=broad-except
