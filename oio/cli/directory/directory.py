...
class DirectoryCmd(Command):
    ...
    def _apply(self, mapping, moved=None, max_attempts=7, read_timeout=M0_READ_TIMEOUT):
        """
...
class DirectoryInit(DirectoryCmd):
    ...
    def get_parser(self, prog_name):
        ...
        parser.add_argument(
            "--force", action="store_true", help="Do the bootstrap even if already done"
        )
        parser.add_argument(
            "--check",
            action="store_true",
...
class DirectoryList(DirectoryCmd):
    ...
    def take_action(self, parsed_args):
        ...
        mapping = self.get_prefix_mapping(parsed_args)
        mapping.load_meta0(
            connection_timeout=M0_CONN_TIMEOUT, read_timeout=parsed_args.meta0_timeout
        )
...
class DirectoryDecommission(DirectoryCmd):
    ...
    def get_parser(self, prog_name):
        ...
        parser.add_argument(
            "addr", metavar="<ADDR>", help="Address of service to decommission"
        )
        parser.add_argument(
            "base", metavar="<BASE>", nargs="*", help="Name of bases to decommission"
        )
        parser.add_argument(
            "--ignore-replicas-number-errors",
            action="store_true",
            help=(
                "Continue even if the number of replicas is not as expected. Dangerous."
            ),
        )
    ...
    def take_action(self, parsed_args):
        ...
        moved = mapping.decommission(parsed_args.addr, bases_to_remove=parsed_args.base)
        if mapping.check_replicas() or parsed_args.ignore_replicas_number_errors:
            self._apply(mapping, moved=moved, read_timeout=parsed_args.meta0_timeout)
...
class DirectoryWarmup(DirectoryCmd):
    ...
    def get_parser(self, prog_name):
        ...
        parser.add_argument(
            "--proxy", type=str, default=None, help="Specific proxy IP:PORT"
        )
        return parser
    ...
    def take_action(self, parsed_args):
        ...
        with green.ContextPool(concurrency) as pool:
            ...
            for _ in range(concurrency):
                worker = WarmupWorker(self.app.client_manager.client_conf, self.log)
