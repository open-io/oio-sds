...
class Meta2Rebuild(SingleServiceCommandMixin, ServiceRebuildCommand):
    ...
    def _take_action(self, parsed_args):
        ...
        self.rebuilder = Meta2Rebuilder(
            self.tool_conf, service_id=parsed_args.service, logger=self.logger
        )
...
class RawxRebuildCommand(SingleServiceCommandMixin, ServiceRebuildCommand):
    ...
    def get_parser(self, prog_name):
        ...
        if not self.distributed:  # local
            parser.add_argument(
                "--dry-run",
                action="store_true",
                help="Display actions but do nothing. (default=%s)"
    ...
    def _take_action(self, parsed_args):
        ...
        if not self.distributed:  # local
            ...
            self.tool_conf["try_chunk_delete"] = parsed_args.delete_faulty_chunks
