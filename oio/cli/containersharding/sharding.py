...
class FindContainerSharding(ContainerShardingCommandMixin, Lister):
    ...
    def patch_parser(parser):
        parser.add_argument(
            ...
            (default: 'rebalance' if the '--all' option is enabled, otherwise 'shard-with-partition')
            """  # noqa: E501
        )
        parser.add_argument(
            '--partition',
            ...
            (default: %s)
            """ % str(ContainerSharding.DEFAULT_PARTITION)
        )
        parser.add_argument(
            '--threshold',
            type=int,
    ...
    def get_parser(self, prog_name):
        ...
        parser.add_argument(
            ...
            WARNING: change the default sharding strategy to 'rebalance'
            """
        )
    ...
    def prepare_startegy(parsed_args):
        ...
        if parsed_args.partition is not None:
            strategy_params['partition'] = json.loads(parsed_args.partition)
