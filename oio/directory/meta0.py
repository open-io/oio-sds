...
def _slice_services(allsrv, level):
    ...
    assert len(allsrv) > 0
    masks = (
        lambda loc: (loc[0], None, None, None),
...
def _bootstrap(allsrv, allgroups, replicas, level, degradation=0):
    ...
    allslices = tuple(((s, e, set()) for s, e in _slice_services(allsrv, level)))
...
class Meta0Client(ProxyClient):
    ...
    def __init__(self, conf, **kwargs):
        super(Meta0Client, self).__init__(conf, request_prefix="/admin", **kwargs)
...
class Meta0PrefixMapping(MetaMapping):
    ...
    def __init__(self, meta0_client, replicas=3, digits=None, min_dist=1, **kwargs):
        """
        ...
        super(Meta0PrefixMapping, self).__init__(meta0_client.conf, ["meta1"], **kwargs)
    ...
    def _compute_upper_limit(self):
        ...
        for location, available_services in available_svcs_by_location.items():
            ideal_bases = (
                float(self.num_bases())
                * self.replicas
        ...
        for svc in sorted(self.services.values(), key=lambda x: x["addr"]):
            ...
            self.logger.info(
                "meta1 %s belongs to location %s and must have a maximum of %d bases",
                svc["addr"],
                svc["location"],
                svc["upper_limit"],
            )
    ...
    def _find_services(self, known=None, lookup=None, max_lookup=50):
        ...
        while len(services) < self.replicas and iterations < max_lookup:
            ...
            for svc in svcs:
                ...
                if all(
                    self.dist_between(loc, loc1) >= self.min_dist
    ...
    def find_services_less_bases(self, known=None, min_score=1, **_kwargs):
        ...
        filtered.sort(key=(lambda x: len(self.get_managed_bases(x))), reverse=True)
    ...
    def decommission(
        ...
        for base in bases_to_remove:
            try:
                ...
                self.logger.warn("Base %s was not managed by %s", base, svc["addr"])
                continue
            try:
    ...
    def rebalance(self, max_loops=4096):
        ...
        self.logger.info(
            "First, find the bases that do not respect the minimum distance"
        )
        ...
        for base, addrs in self.raw_services_by_base.items():
            ...
            for svc_addrs in addrs_by_location.values():
                ...
                svcs = [
                    self.services.get(svc_addr, {"addr": svc_addr})
        ...
        while loops < max_loops and rebalance_again:
            ...
            while candidates:
                ...
                if len(svc_bases) <= upper_limit:
                    ...
                    continue
                self.logger.info(
                    "meta1 %s has %d bases, moving some", svc["addr"], len(svc_bases)
                )
                ...
                if not bases_never_moved and not bases_already_moved:
                    self.logger.info(
                        "meta1 %s has no base available to move", svc["addr"]
                    )
                    continue
                rebalance_again = True

                nb_bases_to_move = len(svc_bases) - upper_limit
                while nb_bases_to_move > 0 and (
                    bases_already_moved or bases_never_moved
                ):
                    ...
                    for base in random.sample(
                        bases_never_moved,
                        min(
                            len(bases_never_moved),
                            nb_bases_to_move - len(bases_to_move),
                        ),
                    ):
                        bases_to_move.add(base)
                        bases_never_moved.remove(base)
                    nb_bases_to_move -= _decommission(svc, bases_to_move, try_=True)
