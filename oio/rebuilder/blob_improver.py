...
class BlobImprover(Rebuilder):
    ...
    def __init__(self, conf, logger, beanstalkd_addr, **kwargs):
        ...
        beanstalkd_tube = self.conf.get("beanstalkd_tube", DEFAULT_IMPROVER_TUBE)
        self.listener = BeanstalkdListener(
    ...
    def _event_from_job(self, job_id, data, **kwargs):
        ...
        if type_ not in self.__class__.supported_events:
            msg = "Discarding event %s (type=%s)" % (event.get("job_id"), type_)
    ...
    def _fill_queue(self, queue, **kwargs):
        ...
        events = self.listener.fetch_jobs(
            self._event_from_job, reserve_timeout=2, **kwargs
        )
        for event in events:
            queue.put(event)
            sent_events += 1
            if max_events > 0 and sent_events >= max_events:
                self.logger.info("Max events (%d) reached, exiting", max_events)
                break
            if not self.running:
                break
        events.close()
    ...
    def _read_retry_queue(self, queue, **kwargs):
        while True:
            ...
            sent = False
            while not sent:
                sent = self.sender.send_job(json.dumps(item), delay=self.retry_delay)
                if not sent:
                    sleep(1.0)
    ...
    def _get_report(self, status, end_time, counters, **kwargs):
        ...
        return (
            ...
            % {
                ...
                "chunks_rate": items_processed / time_since_last_report,
                "errors": errors,
                "errors_rate": 100 * errors / float(items_processed or 1),
                "start_time": datetime.fromtimestamp(int(self.start_time)).isoformat(),
                "total_time": total_time,
                "total_chunks": total_items_processed,
                "total_chunks_rate": total_items_processed / total_time,
...
class BlobImproverWorker(RebuilderWorker):
    ...
    def move_perfectible_from_event(
        self, event, dry_run=False, max_attempts=3, **kwargs
    ):
        """
        ...
        for chunk in moveable:
            try:
                ...
                self.logger.debug("Working on %s: %s", src, chunk.imperfections)
                # TODO(FVE): try to improve all chunks of a metachunk
                # in a single pass
                dst = content.move_chunk(
                    chunk,
                    check_quality=True,
                    dry_run=dry_run,
                    reqid=reqid,
        ...
        if errors:
            if not moves:
                ...
                self.logger.info(
                    ...
                    self.rebuilder._item_to_string(item),
                    errors,
                )
            ...
            for move in moves:
                self.logger.debug(
                    "%s%s moved to %s", "dry-run: " if dry_run else "", move[0], move[1]
                )
        if dry_run:
