...
class EarlyDeleteAgent(IBillingAgent):
    DEFAULT_COUNTER_NAME = "storage.bucket.objects.early-deletion"
    AGENT_NAME = "early-deletion"

    def __init__(self, conf, logger):
        super().__init__(conf, logger)

        self.client = BillingAdjustmentClient(self.conf, logger=self.logger)
    ...
    def list_buckets(self):
        for account, bucket, storage_class in self.client.list_adjustments():
            counters = self.client.reset_adjustment(account, bucket, storage_class)
            yield {
                "account": account,
                "name": bucket,
                "storage_class": storage_class,
                "object_count": counters.get("objects", 0),
                "bytes_used": counters.get("volume", 0),
            }
    ...
    def bucket_to_sample(self, bucket):
        ...
        if bucket["bytes_used"] == 0 and bucket["object_count"] == 0:
            return None
