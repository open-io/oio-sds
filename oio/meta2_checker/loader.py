...
class _Handler(_Type):
    name = 'handler'
    egg_protocols = ['oio.sanity.handler_factory']
    config_prefixes = ['handler']

    def invoke(self, context, **kwargs):
        if context.protocol == 'oio.sanity.handler_factory':
            app = kwargs.pop('app')
...
class _Filter(_Type):
    name = 'filter'
    egg_protocols = ['oio.sanity.filter_factory']
    config_prefixes = ['filter']

    def invoke(self, context, **kwargs):
        if context.protocol == 'oio.sanity.filter_factory':
