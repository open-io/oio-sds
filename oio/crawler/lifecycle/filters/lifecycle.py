...
class Lifecycle(Filter):
    ...
    def _gen_views_non_current_action(self, rule, non_current_days_in_sec, **kwargs):
    ...
    def _exec_rules_non_versioned(
        self, cid, account, container, rules, versioning_enabled, **kwargs
    ):
        ...
        for _, item in mpu_abort_actions.items():
            ...
            self._process_action(
                cid, account, container, rule, action, versioning_enabled, **kwargs
            )
    ...
    def _process_current_action(
        self, cid, account, container, rule, act, versioning_enabled, **kwargs
    ):
        rule_id = rule.id
        days_in_sec = None
        base_sql_query = None
        ...
        self._send_query_events(
            cid, queries, act, view_queries, policy, rule_id, **kwargs
        )
    ...
    def _exec_rules_user_defined_order(
        self, cid, account, container, lc, versioning_enabled, **kwargs
    ):
            ...
            for act in rule.actions:
                if self.is_mpu_container:
                    ...
                    if isinstance(act, AbortIncompleteMultipartUpload):
                        continue
                self._process_action(
                    cid, account, container, rule, act, versioning_enabled, **kwargs
                )
                self.finished_actions += 1
    ...
    def _send_query_events(
        ...
        **kwargs,
        ...
        for key_query, val_query in queries.items():
            offset = self._get_offest(cid, action_type, rule_id, **kwargs)
            while True:
                # Check budget first
