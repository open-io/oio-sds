...
class Lifecycle(Filter):
    ...
    def init(self):
        ...
        self.batch_size = int_value(self.conf.get("lifecycle_batch_size"), 10000)
    ...
    def _get_shard_range(self, cid, **kwargs):
        props = self.container.container_get_properties(cid=cid, **kwargs)
    ...
    def _exec_rules_non_versioned(
        ...
        - Mpu abort (TODO) when adding AbortIncompleteMultipartUpload
        - Expirations
        - Transitions
        """
        # mpu_abort = rules[3]
    ...
    def _process_action(self, cid, account, container, rule, act, versioning_enabled):
        ...
        if versioning_enabled:
            if non_current:
                # NoncurrentVersions, NoncurrentVersions doens't support dates
                if non_current_days is None:
                    self.logger.warning(
    ...
    def _create_views_request(self, cid, view_queries, **kwargs):
        ...
        Views depends on rules adn actions parameters, they
    ...
    def _get_action_parameters(self, act):
        ...
        if isinstance(act, Expiration):
            ...
            elif type(act.filter) == NoncurrentCountActionFilter:
                days = act.filter.days
