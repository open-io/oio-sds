...
static struct oio_events_queue_vtable_s vtable_NATS =
{
	.destroy = _q_destroy,
	.send = _q_send,
...
static _queue_NATS_intercept_error_f intercept_errors = NULL;
#endif
...
GError *
oio_events_queue_factory__create_nats (
...
static gboolean
_q_declare_stream(struct _queue_with_endpoint_s *q,
		struct _running_ctx_s *ctx)
{
	GError *err = nats_declare_stream(ctx->nats->js_ctx, q->queue_name, oio_events_amqp_bind_routing_key);
...
#endif
	if (err) {
		GRID_WARN("Failed to declare Nats stream %s: %s",
...
_q_reconnect(struct _queue_with_endpoint_s *q UNUSED, struct _running_ctx_s *ctx)
{
	...
	EXTRA_ASSERT(ctx->nats != NULL);

	if (ctx->nats && ctx->nats->conn && ctx->nats->js_ctx) {
		return TRUE;
	}

	GError *err = nats_connect(ctx->nats);
#ifdef HAVE_EXTRA_DEBUG
	if (intercept_errors)
		(*intercept_errors) (err);
#endif
...
{
	EXTRA_ASSERT(ctx->nats != NULL && ctx->nats->conn != NULL);
	...
	/* forward the event as a NATS message */
	const size_t msglen = strlen(msg);
	gint64 start = oio_ext_monotonic_time();
	GError *err = nats_send_msg(ctx->nats, msg, msglen, q->routing_key);
	...
	if (!err) {
		...
		if (CODE_IS_RETRY(err->code) || CODE_IS_NETWORK_ERROR(err->code)) {
			GRID_NOTICE("NATS recoverable error with [%s]: (%d) %s",
			...
			GRID_WARN("NATS unrecoverable error with [%s]: (%d) %s",
					q->endpoint, err->code, err->message);
...
_q_run (struct _queue_with_endpoint_s *q)
{
	...
	nats_create(q->endpoint, &(ctx.nats));
	...
	/* close the socket to the nats */
	nats_destroy(ctx.nats);
