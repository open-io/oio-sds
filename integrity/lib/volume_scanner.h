...
typedef enum scanner_traversal_e (*scanner_file_notifier_f) (const gchar *file_path, void *data, struct rules_motor_env_s** motor_env);
 ...
 */
void scan_volume(struct volume_scanning_info_s* scanning_info, struct rules_motor_env_s** motor_env);

/** @} */

/* stamp the last scanned time on a chunk */
void stamp_a_chunk(const char *chunk_path);
